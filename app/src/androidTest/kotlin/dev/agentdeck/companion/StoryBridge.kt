    ...
    init {
        ...
        server.dispatcher = object : Dispatcher() {
            override fun dispatch(request: RecordedRequest): MockResponse {
                ...
                return when {
                    path == "/v1/hello" -> json(requireNotNull(fleetFixture.hello).let { it.copy(capabilities = it.capabilities + MobileProtocol.Capability.ANSWER) }.toJson())
                    path == "/v1/fleet" -> json(snapshot.toJson())
                    path.startsWith("/v1/session/") -> json(page.copy(key = request.requestUrl!!.pathSegments.last()).toJson())
