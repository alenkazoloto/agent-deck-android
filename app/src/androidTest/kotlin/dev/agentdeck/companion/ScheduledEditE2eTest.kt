...
class ScheduledEditE2eTest {
    ...
    @Test fun tapEditAndSaveUsesSameTaskAndKeepsUnsentEditsOnFailure() {
        ...
        // The machine advertises its models, so the field is the catalogue picker. Picked before the
        // delay is typed: a pick closes the keyboard, and the BACK below would then close the dialog.
        ui.onNodeWithText("Model Sonnet 5").performScrollTo().performClick()
        ui.onNodeWithText("Opus 5").performClick()
        ...
        ui.onNodeWithText("Model Opus 5").performScrollTo().assertIsDisplayed()
