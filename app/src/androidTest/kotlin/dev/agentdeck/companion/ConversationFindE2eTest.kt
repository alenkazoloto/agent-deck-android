...
class ConversationFindE2eTest {
    ...
    // Stands in for the keyboard still sliding in after the match was revealed.
    private val lateInset = mutableStateOf(0.dp)
                ...
                AgentDeckTheme(dark = true) {
                    Scaffold(modifier = Modifier.imePadding()) { padding ->
                      Column(Modifier.fillMaxSize().padding(padding)) {
                        ...
                        Box(Modifier.weight(1f)) { ConversationScreen(
                        ...
                        Spacer(Modifier.height(lateInset.value))
                      }
                    }
                }
            }
    ...
    @Test fun revealedMatchStaysAboveTheComposerWhenTheViewportShrinksAfterTheScroll() {
        render()
        ui.onNodeWithText("Find in conversation").performTextInput("needle()")
        awaitCount("1 of 2")
        assertExactPassageVisible(text.indexOf("needle()"))
        ui.runOnIdle { lateInset.value = 100.dp }
        assertExactPassageVisible(text.indexOf("needle()"))
    }
