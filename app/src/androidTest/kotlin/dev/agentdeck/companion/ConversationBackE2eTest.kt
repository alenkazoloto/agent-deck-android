...
class ConversationBackE2eTest {
    ...
    @Test fun savedCopyAgeStaysVisibleAtLongTailUntilThisPageRefreshes() {
        val old = bridge.pages.getValue(first.key).copy(
            generatedAtMs = System.currentTimeMillis() - 86_400_000,
            turns = (0..40).map { turn("saved-$it", "Saved passage $it. " + "Readable history. ".repeat(15)) },
        )
        store.cacheTranscript(bridge.machine.id, old)
        val gate = bridge.holdNextTranscript(first.key).also { gates += it }
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        ui.waitUntil(10_000) { gate.received.count == 0L }
        ui.onNodeWithText("Saved copy · as of", substring = true).assertIsDisplayed()
        ...
        screenshot("conversation-saved-tail-keyboard")
        gate.release()
        awaitText(firstBody)
        ui.onNodeWithText("Saved copy · as of", substring = true).assertDoesNotExist()
        ui.onNode(hasSetTextAction()).assertTextContains(draft)
    }

    @Test fun failedRefreshKeepsSavedAgeEvenWithRecentFleet() {
        bridge.unavailable = true
        store.cacheTranscript(bridge.machine.id, bridge.pages.getValue(first.key).copy(
            generatedAtMs = System.currentTimeMillis() - 86_400_000,
        ))
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText(firstBody)
    ...
    @Test fun earlierHistoryUsesNegotiatedCursorAndKeepsBoundaryVisible() {
        prepareHistory()
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText("History passage 239")
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        ui.waitUntil(10_000) { "history-before" in bridge.earlierReads }
        ui.waitUntil(10_000) { store.cachedTranscript(bridge.machine.id, first.key)?.turns?.size == 240 }
        ui.onNodeWithText("History passage 120").assertIsDisplayed()
        ui.onNodeWithContentDescription("Latest message").assertIsDisplayed()
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithText("History passage 0").assertIsDisplayed()
        ui.onNodeWithContentDescription("Load earlier messages").assertDoesNotExist()
        screenshot("conversation-earlier-history")
    }

    @Test fun expiredHistoryHasLocalRefreshWithoutClearingTheReadPage() {
        prepareHistory()
        bridge.expireHistory = true
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText("History passage 239")
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        awaitText("History changed. Refresh to load earlier messages.")
        ui.onNodeWithText("History passage 120").assertIsDisplayed()
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Refresh conversation history").assertIsDisplayed().performClick()
        ui.waitUntil(10_000) { ui.onAllNodesWithText("History changed. Refresh to load earlier messages.").fetchSemanticsNodes().isEmpty() }
        ui.onNode(hasSetTextAction()).assertTextContains(draft)
    }

    @Test fun earlierHistoryRetriesOfflineAndContinuesBoundedAncestryScan() {
        prepareHistory()
        val finalPage = bridge.earlierPages.getValue("history-before")
        bridge.earlierPages["history-before"] = finalPage.copy(turns = emptyList(), previousCursor = "scan-next", hasMore = true, historyPending = true)
        bridge.earlierPages["scan-next"] = finalPage
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText("History passage 239")
        bridge.unavailable = true
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        ui.waitUntil(10_000) { ui.onAllNodesWithContentDescription("Retry earlier messages").fetchSemanticsNodes().isNotEmpty() }
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        bridge.unavailable = false
        ui.onNodeWithContentDescription("Retry earlier messages").performClick()
        ui.waitUntil(10_000) { store.cachedTranscript(bridge.machine.id, first.key)?.turns?.size == 240 }
        assertEquals(listOf("history-before", "scan-next"), bridge.earlierReads.toList())
        ui.onNodeWithText("History passage 120").assertIsDisplayed()
    }

    @Test fun lateEarlierResponseCannotReplaceTheNotificationConversation() {
        prepareHistory()
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        scenario!!.onActivity { launchedActivity = it; originalIntent = Intent(it.intent) }
        awaitText("History passage 239")
        val gate = bridge.holdNextTranscript(first.key).also { gates += it }
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        ...
        val observeUntil = SystemClock.uptimeMillis() + 1_000
        ui.waitUntil(3_000) {
            ui.onNodeWithText(secondBody).assertIsDisplayed()
            ui.onNodeWithText("History passage 0").assertDoesNotExist()
            SystemClock.uptimeMillis() >= observeUntil
        }
        assertEquals(120, store.cachedTranscript(bridge.machine.id, first.key)?.turns?.size)
    }

    @Test fun failedForcedHistoryRefreshKeepsDownloadedMessagesAndDraft() {
        prepareHistory()
        bridge.expireHistory = true
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText("History passage 239")
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        ui.waitUntil(10_000) { ui.onAllNodesWithContentDescription("Refresh conversation history").fetchSemanticsNodes().isNotEmpty() }
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        bridge.unavailable = true
        ui.onNodeWithContentDescription("Refresh conversation history").performClick()
        ui.waitUntil(10_000) { ui.onAllNodesWithText("Machine unavailable").fetchSemanticsNodes().isNotEmpty() }
        ui.onNodeWithText("History passage 120").assertIsDisplayed()
        ui.onNode(hasSetTextAction()).assertTextContains(draft)
    }

    @Test fun boundedScanPausesUntilExplicitContinuation() {
        prepareHistory()
        val finalPage = bridge.earlierPages.getValue("history-before")
        bridge.earlierPages["history-before"] = finalPage.copy(turns = emptyList(), previousCursor = "paused-next", hasMore = true,
            historyPending = true, historyPaused = true)
        bridge.earlierPages["paused-next"] = finalPage
        scenario = ActivityScenario.launch(Intent(app, MainActivity::class.java))
        awaitText("History passage 239")
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Load earlier messages").performClick()
        ui.waitUntil(10_000) { ui.onAllNodesWithContentDescription("Continue loading history").fetchSemanticsNodes().isNotEmpty() }
        assertEquals(listOf("history-before"), bridge.earlierReads.toList())
        ui.onNodeWithTag("conversation-transcript").performScrollToIndex(0)
        ui.onNodeWithContentDescription("Continue loading history").performClick()
        ui.waitUntil(10_000) { store.cachedTranscript(bridge.machine.id, first.key)?.turns?.size == 240 }
        assertEquals(listOf("history-before", "paused-next"), bridge.earlierReads.toList())
    }

    private fun prepareHistory() {
        bridge.paging = true
        val fresh = bridge.pages.getValue(first.key).copy(revision = "history-r1", previousCursor = "history-before", hasMore = true,
            turns = (120..239).map { turn("history-$it", "History passage $it") })
        bridge.pages[first.key] = fresh
        bridge.earlierPages["history-before"] = fresh.copy(previousCursor = null, hasMore = false,
            turns = (0..120).map { turn("history-$it", "History passage $it") })
        store.cacheTranscript(bridge.machine.id, fresh)
    }
