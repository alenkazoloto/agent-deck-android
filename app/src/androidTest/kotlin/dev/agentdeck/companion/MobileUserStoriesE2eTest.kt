...
class MobileUserStoriesE2eTest {
    ...
    @Test fun searchOpenAndReplyUsesRealBridgeAndClearsOnlyAcceptedDraft() {
        ...
        // The run line opens on the desk's selection; the follow-up changes one field in its sheet.
        awaitDescription("Run settings: ", substring = true)
        ui.onNodeWithContentDescription("Run settings: ", substring = true).assertTextContains("Opus 5", substring = true)
            .performClick()
        ui.onNode(hasText("Default effort") and hasAnyAncestor(hasTestTag("run-settings-sheet"))).performClick()
        ui.onNodeWithText("Low effort", substring = false).performClick()
        // The first Back may only close the keyboard still open under the sheet.
        repeat(3) {
            if (ui.onAllNodesWithTag("run-settings-sheet").fetchSemanticsNodes().isNotEmpty()) {
                InstrumentationRegistry.getInstrumentation().sendKeyDownUpSync(android.view.KeyEvent.KEYCODE_BACK)
                ui.waitForIdle()
            }
        }
        ui.waitUntil(5_000) { ui.onAllNodesWithTag("run-settings-sheet").fetchSemanticsNodes().isEmpty() }
        awaitText("Navigation review is ready.")
        ui.onNodeWithContentDescription("Run settings: ", substring = true).assertTextContains("Low effort", substring = true)
