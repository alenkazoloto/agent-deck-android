...
class DeskSelectionSendTest {
    ...
    @Test fun `an untouched composer names no field and leaves every field to the desk`() {
        ...
        assertEquals(Field.entries.toSet(), desk)
    }
    ...
    @Test fun `only what was picked is named, and a picked Default stays a pick`() {
        ...
        assertEquals("a picked Default was handed back to the desk", setOf(Field.MODE, Field.FAST, Field.THINKING), desk)
    }
