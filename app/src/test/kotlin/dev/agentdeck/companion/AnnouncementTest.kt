...
class AnnouncementTest {
    ...
    /** Each reason reads differently; none of them is the bare group title. */
    @Test
    fun `a waiting row names what it waits for`() {
        val said = WaitingReason.entries.map { rowAnnouncement(row(waitingReason = it), FleetGroup.WAITING) }
        assertEquals(
            "Claude conversation, Fix the parser, Waiting for tool permission, in plugin",
            said[WaitingReason.PERMISSION.ordinal],
        )
        assertEquals(WaitingReason.entries.size, said.toSet().size)
        assertTrue(said.none { it.contains(FleetGroup.WAITING.title) })
    }
