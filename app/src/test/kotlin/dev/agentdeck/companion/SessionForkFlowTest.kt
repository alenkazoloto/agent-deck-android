...
class SessionForkFlowTest {
    ...
    /** Every other route asked, with its body. */
    private val other = CopyOnWriteArrayList<Pair<String, String>>()
    ...
    @Test fun `a Claude chat branches whole at once, opening the copy with nothing in its composer`() {
        model.branchChat(ROW)
        eventually { (model.state.value.screen as? Screen.Conversation)?.takeIf { it.key == FORK_KEY } }

        assertNull(model.state.value.forkPoints)
        assertEquals(1, asked.size)
        assertTrue("the desk's Branch chat copies the whole conversation", asked.single().whole)
        assertNull(asked.single().point)
        assertTrue("a branch names an operation", !asked.single().operationId.isNullOrBlank())
        assertEquals("", model.draft(FORK_KEY))
    }

    @Test fun `branch typed in a conversation branches that chat whole`() {
        model.openConversation(CODEX)
        model.deskNavigation(com.github.claudeagents.core.mobile.MobileDeskCommands.Action.BRANCH)
        eventually { (model.state.value.screen as? Screen.Conversation)?.takeIf { it.key == FORK_KEY } }

        assertEquals(CODEX.key, asked.single().key)
        assertTrue(asked.single().whole)
    }

    @Test fun `rename typed in a conversation renames that chat through session-actions`() {
        model.renameConversation(ROW.key, "Parser fix")
        val (_, body) = eventually { other.firstOrNull { it.first == com.github.claudeagents.core.mobile.MobileSessionActionRequest.ROUTE } }

        val request = com.github.claudeagents.core.mobile.MobileSessionActionRequest.fromJson(MobileProtocol.parseObject(body)!!)
        assertEquals(com.github.claudeagents.core.mobile.MobileSessionActionRequest(ROW.key, com.github.claudeagents.core.mobile.MobileSessionActionRequest.RENAME, "Parser fix"), request)
    }

    @Test fun `export typed in a conversation asks the machine for that chat's export`() {
        model.openConversation(ROW)
        model.deskNavigation(com.github.claudeagents.core.mobile.MobileDeskCommands.Action.EXPORT)
        eventually { other.firstOrNull { it.first == com.github.claudeagents.core.mobile.MobileSessionExport.ROUTE } }
    }

    @Test fun `personality typed in a Codex conversation opens that chat's Codex settings`() {
        model.openConversation(CODEX)
        model.deskNavigation(com.github.claudeagents.core.mobile.MobileDeskCommands.Action.CODEX_SETTINGS)

        assertEquals(CODEX.key, model.sessionCodex.sheet.value?.key)
        eventually { other.firstOrNull { it.first.startsWith(com.github.claudeagents.core.mobile.MobileSessionCodexQuery.ROUTE) } }
    }

    @Test fun `branch typed in a chat the list does not hold yet asks nothing and says so`() {
        model.openConversation(ROW.copy(key = "claude:/repo:unlisted"))
        model.deskNavigation(com.github.claudeagents.core.mobile.MobileDeskCommands.Action.BRANCH)
        eventually { model.state.value.snack?.message?.takeIf { it.startsWith("This chat is not in the machine's list yet") } }
    ...
    private fun bridge() = BridgeClient(listOf("a.test"), 8443, "00", "token") { url ->
        object : HttpsURLConnection(url) {
            ...
            override fun getResponseCode(): Int {
                if (url.path != MobileSessionForkRequest.ROUTE) {
                    other += url.path to body.toString()
                    return 404
                }
