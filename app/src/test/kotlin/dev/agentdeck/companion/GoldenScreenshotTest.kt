...
class GoldenScreenshotTest {
    ...
    @Test fun `scheduled, recent runs`() = capture("scheduled-outcomes", Shape.PORTRAIT) { Scheduled(outcomes = true) }

    @Test fun `scheduled, recent runs, dark`() = capture("scheduled-outcomes-dark", Shape.DARK) { Scheduled(outcomes = true) }

    @Test fun `scheduled, created outside the IDE`() = capture("scheduled-outside", Shape.PORTRAIT) { Scheduled(outside = true) }

    @Test fun `scheduled, created outside the IDE, dark`() = capture("scheduled-outside-dark", Shape.DARK) { Scheduled(outside = true) }

    @Test fun `scheduled, follow-ups`() = capture("scheduled-follow-ups", Shape.PORTRAIT) { Scheduled(followUps = true) }

    @Test fun `scheduled, follow-ups, dark`() = capture("scheduled-follow-ups-dark", Shape.DARK) { Scheduled(followUps = true) }

    @Test fun `scheduled, overlapping runs off`() = capture("scheduled-overlap-off", Shape.PORTRAIT) { Scheduled(allowOverlap = false) }

    @Test fun `scheduled, overlapping runs on, dark`() = capture("scheduled-overlap-on-dark", Shape.DARK) { Scheduled(allowOverlap = true) }
    ...
    /** M2: a draft brings the run-settings line, spelling what the desk would send next. */
    ...
    private fun Conversation(
        ...
        ConversationScreen(
            ...
            onSearchFiles = { MentionMatches(listOf("ui/ConversationScreen.kt", "ui/ConversationFind.kt")) },
        )
    }
    ...
    @Composable
    private fun Scheduled(empty: Boolean = false, outcomes: Boolean = false, outside: Boolean = false, followUps: Boolean = false, allowOverlap: Boolean? = null) {
        val state = DeckFixtures.byName("scheduled")!!
        ScheduledScreen(
            outcomes = if (outcomes) listOf(
                MobileScheduledOutcome("t1", "Run the regression tests", "/Users/dev/Plugin", "v2:key", AgentVendor.CLAUDE, DeckFixtures.NOW - 12 * 60_000, false, null),
                MobileScheduledOutcome("t2", "Nightly dependency audit", "/Users/dev/Plugin", "v2:key2", AgentVendor.CLAUDE, DeckFixtures.NOW - 3 * 3_600_000, true, "exit 1: claude: command not found"),
                MobileScheduledOutcome("t3", "Summarise the release branch", "/Users/dev/Site", null, null, DeckFixtures.NOW - 26 * 3_600_000, false, null),
            ) else emptyList(),
            outside = if (outside) listOf(
                MobileScheduledOutside("codex:funda", "Codex", "Paused — Daily at 09:00", "User", "Search for properties matching buying criteria.", "memory.md"),
                MobileScheduledOutside("j1", "Claude Code", "Every 5 min", "Project", "Poll the CI build and report failures"),
                MobileScheduledOutside("routine:triage", "Cloud routine", "Cloud trigger", "User", "Triage new issues each morning"),
            ) else emptyList(),
            followUps = if (followUps) listOf(
                MobileScheduledFollowUp(
                    "f1", "Finished", "/Users/dev/Plugin", "v2:result", AgentVendor.CLAUDE,
                    listOf(
                        MobileFollowUpSource("Implementation session", "finished", true, "v2:a", AgentVendor.CLAUDE),
                        MobileFollowUpSource("Test session", "finished", false, "v2:b", AgentVendor.CLAUDE),
                    ),
                ),
                MobileScheduledFollowUp(
                    "f2", "Failed", "/Users/dev/Site", null, null,
                    listOf(MobileFollowUpSource("Release notes draft", "unconfirmed", false, null, null)),
                ),
            ) else emptyList(),
