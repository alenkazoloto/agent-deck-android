...
class GoldenScreenshotTest {
    ...
    /**
     * The self-update offer, against the `settings` shot above as its negative control: the two
     * fixtures differ in the published `versionCode` and in nothing else, so "Up to date." and
     * the Download row cannot be the same picture.
     */
    @Test fun `settings, an update is published`() =
        capture("settings-update-font-1_3", Shape.LARGE_TEXT) { Settings("settings-update") }
