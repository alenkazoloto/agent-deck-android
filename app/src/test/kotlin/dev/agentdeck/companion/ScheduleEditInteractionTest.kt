...
class ScheduleEditInteractionTest {
    ...
    @Test fun `Fast and Thinking picks ride the save only once touched, and only from a host that applies them`() {
        val hello = MobileHello(
            protocolVersion = MobileProtocol.VERSION, machineName = "m", ideName = "IDE", pluginVersion = "1",
            capabilities = listOf(MobileProtocol.Capability.RUN_TOGGLES),
        )
        fun detail(editable: Boolean) = MobileScheduleEditDetail("task", "Review patch", "/work/project", "chat", 1_800_000_000_000,
            0, null, "opus", "CLAUDE", "personal", null, "Europe/Amsterdam", true, false,
            fastMode = true, runTogglesEditable = editable)
        val saved = mutableListOf<MobileScheduleEditRequest>()
        val shown = mutableStateOf(detail(true))
        ui.setContent {
            AgentDeckTheme {
        ...
        ui.onNodeWithText("Save").performClick()
        ui.runOnIdle {
            assertFalse("an untouched form rewrote the row's toggles", saved.single().editRunToggles)
            assertEquals(true, saved.single().fastMode)
        }
        ui.onNodeWithContentDescription("Thinking: Default").performScrollTo().performClick()
        ui.onNodeWithText("Off").performClick()
        ui.onNodeWithText("Save").performClick()
        ui.runOnIdle {
            assertTrue(saved.last().editRunToggles)
            assertEquals(false, saved.last().thinking)
            assertEquals("the untouched Fast pick must ride along", true, saved.last().fastMode)
        }
        shown.value = detail(false)
        ui.onNodeWithTag("schedule-edit-fast").assertDoesNotExist()
        ui.onNodeWithTag("schedule-edit-thinking").assertDoesNotExist()
    }
}
