...
class RunningIndicatorInteractionTest {
    ...
    @Test
    fun `swiping a running row left never stops it, while swiping right still snoozes`() {
        val fixture = DeckFixtures.byName("fleet-uncapped")!!.snapshot!!
        val row = fixture.rows.first().copy(
            title = "Review the parser", attention = SessionAttentionState.RUNNING,
            liveLine = "Running tests", lastActivityMs = fixture.generatedAtMs,
        )
        ...
        compose.setContent {
            AgentDeckTheme {
                FleetScreen(
                    snapshot = fixture.copy(rows = listOf(row)), filter = FleetFilter(), sort = FleetSort.RECENT,
                    refreshing = false, snoozed = emptyMap(), openKey = null,
                    onFilter = {}, onSort = {}, onRefresh = {}, onOpen = {},
                    onSnooze = { snoozed += it.key }, onStop = { stopped += it.key },
                )
            }
        }
        val rowNode = compose.onNodeWithContentDescription("Review the parser", substring = true)

        rowNode.performTouchInput { swipeLeft() }
        compose.waitForIdle()
        assertEquals(emptyList<String>(), stopped)

        rowNode.performTouchInput { swipeRight() }
        compose.waitForIdle()
        // The row stays put here (nothing filters it out), so the snooze may repeat as the box settles.
        assertTrue(snoozed.isNotEmpty() && snoozed.all { it == row.key })
    }
