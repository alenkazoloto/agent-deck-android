...
class ConversationInteractionTest {
    ...
    private fun show(
        ...
        turns: List<MobileTurn> = emptyList(),
        agentName: String? = null,
        onStopTask: ((String, String) -> Unit)? = null,
    ) {
    ...
    @Test
    fun `background tasks are listed read-only, the rest folded into a count, and the note goes with them`() {
        show(running = false)
        compose.onNodeWithText("1 background task").assertDoesNotExist()

        val tasks = listOf(
            MobileBackgroundTask("Agent", "Review changes"),
            MobileBackgroundTask("Shell command", null),
            MobileBackgroundTask("Monitor", "Watch the deploy"),
            MobileBackgroundTask("Shell command", "npm test"),
            MobileBackgroundTask("Agent", "Fifth"),
        )
        compose.runOnIdle { pageState.value = pageState.value!!.copy(backgroundTasks = tasks) }
        compose.onNodeWithText("5 background tasks").assertIsDisplayed()
        compose.onNodeWithText("Agent · Review changes").assertIsDisplayed()
        compose.onNodeWithText("Shell command").assertIsDisplayed()
        compose.onNodeWithText("+1 more").assertIsDisplayed()

        compose.runOnIdle { pageState.value = pageState.value!!.copy(backgroundTasks = emptyList()) }
        compose.onNodeWithText("5 background tasks").assertDoesNotExist()
    }

    @Test
    fun `a task the machine named an id for has a Stop that sends that id, and one it did not stays read-only`() {
        val stopped = mutableListOf<Pair<String, String>>()
        show(running = false, onStopTask = { id, label -> stopped += id to label })
        val tasks = listOf(
            MobileBackgroundTask("Agent", "Review changes", "a1"),
            MobileBackgroundTask("Shell command", "npm test"),
            MobileBackgroundTask("Monitor", "Watch the deploy", "c3"),
        )
        compose.runOnIdle { pageState.value = pageState.value!!.copy(backgroundTasks = tasks) }

        compose.onNodeWithText("3 background tasks").assertIsDisplayed()
        compose.onAllNodesWithText("Stop").assertCountEquals(2)
        compose.onNodeWithContentDescription("Stop Monitor · Watch the deploy").performClick()

        assertEquals(listOf("c3" to "Monitor · Watch the deploy"), stopped)
        compose.onNodeWithContentDescription("Stop Shell command · npm test").assertDoesNotExist()
    }

    @Test
    fun `without the capability every task is read-only even when the page carries ids`() {
        show(running = false, onStopTask = null)
        compose.runOnIdle {
            pageState.value = pageState.value!!.copy(backgroundTasks = listOf(MobileBackgroundTask("Agent", "Review changes", "a1")))
        }
