...
class UsageFetchTest {
    ...
    /** The query string of each of those, `""` for none. */
    ...
    /**
     * The Usage chips' pick travels with the fetch — the desk tab's Agent and Account filters cut on the
     * machine, so a phone that only remembered the pick would repaint the unfiltered spend under a
     * chip that says otherwise. And the machine's echo, not the ask, becomes the state: an account it
     * no longer offers comes back as "all" and must not be asked for again on every refresh.
     */
    @Test fun `a chosen agent and account ride the fetch and the machine's echo replaces a stale pick`() = runBlocking {
        store.saveScreen(Navigation.toJson(Screen.Usage))
        val model = model()
        settle()
        model.refreshHello()
        ...
        model.filterUsage(AgentVendor.CODEX, "codex")
        settle()
        assertEquals(listOf("agent=CODEX&account=codex"), queries.filter { it.isNotEmpty() })
        assertEquals(AgentVendor.CODEX, model.state.value.usage!!.filter.agent)
        assertEquals("codex", model.state.value.usageAccount)

        queries.clear()
        model.filterUsage(AgentVendor.CODEX, "gone")
        settle()
        assertEquals("the ask names the pick the phone held", listOf("agent=CODEX&account=gone"), queries)
        assertNull("the machine no longer offers it, so the phone stops asking for it", model.state.value.usageAccount)
        assertEquals(AgentVendor.CODEX, model.state.value.usageAgent)

        queries.clear()
        model.loadUsage()
        settle()
        assertEquals("a refresh keeps the agent and no longer sends the dropped account", listOf("agent=CODEX"), queries)
    }

    /**
     * Session insights count what the figures behind them count: the sheet asks `/v1/insights` with
     * the screen's own Agent / Account pick, so a Codex cut never reads as the whole machine.
     */
    @Test fun `session insights are asked for under the pick the usage screen holds`() = runBlocking {
        store.saveScreen(Navigation.toJson(Screen.Usage))
        val model = model()
        settle()
        model.refreshHello()
        ...
        assertEquals(listOf("/v1/insights"), asked)
        assertEquals(listOf("agent=CODEX&account=codex"), queries)
    }
        ...
        object : HttpsURLConnection(url) {
            ...
            override fun getInputStream(): InputStream {
                ...
                return when {
                    ...
                    url.path.endsWith(MobileUsageReport.ROUTE) -> usage(url.query.orEmpty())
                    else -> "{}"
                }.byteInputStream()
            }
        }
    ...
    /** Echoes only the account this machine still offers, as the route does: "gone" is an account it dropped. */
    private fun usage(query: String): String {
        val asked = query.split('&').mapNotNull { it.split('=', limit = 2).takeIf { p -> p.size == 2 }?.let { p -> p[0] to p[1] } }.toMap()
        return MobileUsageReport(
            ...
            filter = MobileUsageFilter(
                ...
                accounts = listOf(MobileUsageFilterAccount("codex", "Work (Codex)")),
                agent = AgentVendor.entries.firstOrNull { it.name == asked[MobileUsageReport.AGENT_PARAM] },
                account = asked[MobileUsageReport.ACCOUNT_PARAM]?.takeIf { it == "codex" },
            ),
