...
class QuestionAnswersTest {
    ...
    private val sentTyped = mutableListOf<Set<String>>()

    private fun show(fixture: String, answerFailures: Int = 0, typing: Boolean = false) {
            ...
            CompositionLocalProvider(LocalNow provides { DeckFixtures.NOW }) {
                AgentDeckTheme {
                    ConversationScreen(
                        ...
                        onDismissNotice = {},
                        canAnswer = true,
                        onAnswer = { _, answers -> sent += answers },
                        onAnswerTyped = { _: String, answers: Map<String, String>, typed: Set<String> -> sent += answers; sentTyped += typed }.takeIf { typing },
    ...
    @Test
    fun `Something else sends the typed words marked as typed`() {
        show("convo-question", typing = true)
        tap("Something else")
        compose.onNodeWithText("Send answer").performScrollTo().assertIsNotEnabled()
        compose.onNodeWithText("Your answer").performScrollTo().performTextInput("Pin it, and add a retry")
        compose.onNodeWithText("Send answer").performScrollTo().assertIsEnabled().performClick()
        assertEquals(listOf(mapOf("What should the flaky-test fix cover?" to "Pin it, and add a retry")), sent)
        assertEquals(listOf(setOf("What should the flaky-test fix cover?")), sentTyped)
    }

    @Test
    fun `a machine without typed answers is not offered a text field`() {
        show("convo-question")
        compose.onNodeWithText("Something else").assertDoesNotExist()
    }
