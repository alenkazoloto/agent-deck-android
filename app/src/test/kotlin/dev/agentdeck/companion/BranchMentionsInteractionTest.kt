...
/**
 * Settings › Machine's "Branches after @" is the desk's switch: drawn only once the machine
 * has answered with its value, asked for on every visit, and a tap asks for the opposite.
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [34], qualifiers = "w411dp-h891dp")
class BranchMentionsInteractionTest {
    ...
    private fun show(branchMentions: Boolean?) = compose.setContent {
        AgentDeckTheme(dark = false, dynamic = false) {
            SettingsScreen(
                state = DeckFixtures.byName("settings")!!.copy(keepAwake = true, branchMentions = branchMentions),
                onSettings = {},
                onSwitchMachine = {},
                onAddMachine = {},
                onUnpair = {},
                ...
                onRefreshBranchMentions = { reads++ },
                onBranchMentions = { flips += it },
            )
        }
    }
    ...
    fun `the switch asks for the opposite and leaves keep awake alone`() {
        show(branchMentions = true)

        compose.onNodeWithText("Branches after @").assertIsDisplayed().performClick()
    ...
    fun `an off switch asks for on`() {
        show(branchMentions = false)

        compose.onNodeWithText("Branches after @").performClick()
    ...
    fun `a machine that has not answered draws no switch`() {
        show(branchMentions = null)

        assertEquals(0, count("Branches after @"))
    }
