...
class ReviewCommitFlowTest {
    ...
    @Test fun `a request's commit ticks its files alone, over saved ticks, and a stale re-read never widens it`() {
        preview = PREVIEW.copy(files = PREVIEW.files + MobileReviewCommitFile("src/B.kt", MobileReviewCommitFile.MODIFIED, true))
        flow.open(KEY)
        eventually { flow.sheet.value?.preview }
        flow.toggle("src/New.kt")
        flow.dismiss()

        flow.open(KEY, only = setOf("src/B.kt", "src/Gone.kt"))
        val sheet = eventually { flow.sheet.value?.takeIf { it.preview != null } }
        assertEquals("the request's file the machine can commit, not the saved ticks", setOf("src/B.kt"), sheet.chosen)

        preview = preview.copy(previewToken = "t2")
        answers += MobileRefusal.COMMIT_PREVIEW_STALE.status to MobileRefusal.COMMIT_PREVIEW_STALE.toJson().toString()
        flow.confirm()
        val fresh = eventually { flow.sheet.value?.takeIf { it.preview?.previewToken == "t2" } }
        assertEquals(setOf("src/B.kt"), fresh.chosen)
    }
