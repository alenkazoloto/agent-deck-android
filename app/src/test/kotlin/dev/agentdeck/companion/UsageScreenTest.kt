...
class UsageScreenTest {
    ...
    /** The defaults are the same power as a chat's own limits, so the row follows the machine's advertisement, not the build. */
    @Test
    fun `Edit spend defaults appears only on a machine that advertises it, and asks for the defaults`() {
        var edited = 0
        val withDefaults = DeckFixtures.byName("settings-usage")!!.hello!!.let { it.copy(capabilities = it.capabilities + MobileProtocol.Capability.SESSION_SPEND_DEFAULTS) }
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage!!.copy(caps = emptyList()), loading = false, error = null, onLoad = {}, hello = withDefaults, onEditSpendDefaults = { edited++ })
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasTestTag("usage-edit-spend-defaults"))
        compose.onNodeWithText("Spend limits").assertIsDisplayed()
        compose.onNodeWithTag("usage-edit-spend-defaults").assertIsDisplayed().performClick()
        assertEquals(1, edited)
    }

    @Test
    fun `no Edit spend defaults row on a machine that does not advertise it`() {
        val older = DeckFixtures.byName("settings-usage")!!.hello!!
        assert(MobileProtocol.Capability.SESSION_SPEND_DEFAULTS !in older.capabilities) { "the negative control must not advertise it" }
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage, loading = false, error = null, onLoad = {}, hello = older, onEditSpendDefaults = { error("offered") })
            }
        }
        assertEquals(0, compose.onAllNodesWithTag("usage-edit-spend-defaults").fetchSemanticsNodes().size)
    }

    /**
     * The desk's "Set active": offered on the Claude account that is not active, never on the active
     * one, and never on Codex's only account (the desk lists choices from the second account of an
     * agent on). The tap names the agent and the account it pins.
     */
    @Test
    fun `Set active is offered on the other account of an agent and names it`() {
        val picks = mutableListOf<Pair<AgentVendor, String>>()
        val hello = DeckFixtures.byName("usage-set-active")!!.hello
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage, loading = false, error = null, onLoad = {}, hello = hello, onSetActiveAccount = { v, id -> picks += v to id })
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasTestTag("usage-set-active-CLAUDE-personal"))
        compose.onNodeWithTag("usage-set-active-CLAUDE-personal").assertIsDisplayed().performClick()

        assertEquals(listOf(AgentVendor.CLAUDE to "personal"), picks)
        assertEquals("the active account is already the pin", 0, compose.onAllNodesWithTag("usage-set-active-CLAUDE-work").fetchSemanticsNodes().size)
        assertEquals("one Codex account has nothing to switch to", 0, compose.onAllNodesWithTag("usage-set-active-CODEX-codex").fetchSemanticsNodes().size)
    }

    @Test
    fun `no Set active on a machine that does not advertise it`() {
        val older = DeckFixtures.byName("settings-usage")!!.hello!!
        assert(MobileProtocol.Capability.ACTIVE_ACCOUNT !in older.capabilities) { "the negative control must not advertise it" }
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage, loading = false, error = null, onLoad = {}, hello = older, onSetActiveAccount = { _, _ -> error("offered") })
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasText("Personal"))
        assertEquals(0, compose.onAllNodesWithTag("usage-set-active-CLAUDE-personal").fetchSemanticsNodes().size)
    }

    /** The desk's Accounts › Rename under the per-phone grant: a rename names the typed, trimmed name. */
    @Test
    fun `Rename names the account and the typed name`() {
        val edits = mutableListOf<MobileAccountEdit>()
        val hello = DeckFixtures.byName("usage-account-edit")!!.hello
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage, loading = false, error = null, onLoad = {}, hello = hello, onEditAccount = { edits += it })
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasTestTag("usage-account-rename-CLAUDE-personal"))
        compose.onNodeWithTag("usage-account-rename-CLAUDE-personal").performClick()
        compose.onNodeWithTag("usage-account-rename-save").assertIsNotEnabled()
        compose.onNodeWithTag("usage-account-rename-field").performTextReplacement("  Home  ")
        compose.onNodeWithTag("usage-account-rename-save").performClick()
        assertEquals(listOf(MobileAccountEdit(AgentVendor.CLAUDE, "personal", MobileAccountEdit.Op.RENAME, "Home")), edits)
    }

    /** Remove asks first and says what stays on disk; nothing is sent before the confirmation. */
    @Test
    fun `Remove asks first and names the account`() {
        val edits = mutableListOf<MobileAccountEdit>()
        val hello = DeckFixtures.byName("usage-account-edit")!!.hello
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
        ...
        compose.onNode(hasScrollAction()).performScrollToNode(hasTestTag("usage-account-remove-CLAUDE-work"))
        compose.onNodeWithTag("usage-account-remove-CLAUDE-work").performClick()
        compose.onNodeWithText("The machine forgets this account. Its chats and files stay on disk.").assertIsDisplayed()
        assertEquals("nothing is sent before the confirmation", emptyList<MobileAccountEdit>(), edits)
        compose.onNodeWithTag("usage-account-remove-confirm").performClick()
        assertEquals(listOf(MobileAccountEdit(AgentVendor.CLAUDE, "work", MobileAccountEdit.Op.REMOVE)), edits)
    }

    @Test
    fun `an agent's default account has no Remove`() {
        val hello = DeckFixtures.byName("usage-account-edit")!!.hello
        val report = state.usage!!.let { r -> r.copy(accounts = r.accounts.map { if (it.id == "personal") it.copy(removable = false) else it }) }
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = report, loading = false, error = null, onLoad = {}, hello = hello)
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasTestTag("usage-account-rename-CLAUDE-personal"))
        compose.onNodeWithTag("usage-account-rename-CLAUDE-personal").assertIsDisplayed()
        assertEquals(0, compose.onAllNodesWithTag("usage-account-remove-CLAUDE-personal").fetchSemanticsNodes().size)
    }

    @Test
    fun `no Rename or Remove on a machine that does not advertise them`() {
        val older = DeckFixtures.byName("settings-usage")!!.hello!!
        assert(MobileProtocol.Capability.ACCOUNT_EDITS !in older.capabilities) { "the negative control must not advertise it" }
        compose.setContent {
            AgentDeckTheme(dark = false, dynamic = false) {
                UsageScreen(report = state.usage, loading = false, error = null, onLoad = {}, hello = older, onEditAccount = { error("offered") })
            }
        }
        compose.onNode(hasScrollAction()).performScrollToNode(hasText("Personal"))
        assertEquals(0, compose.onAllNodesWithTag("usage-account-rename-CLAUDE-personal").fetchSemanticsNodes().size)
    }
