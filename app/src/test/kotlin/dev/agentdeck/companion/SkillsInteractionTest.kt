...
/**
 * Settings › Resources › "Skills and agents" is the desk's skills, commands and subagents, read-only:
...
class SkillsInteractionTest {
    ...
    private fun open() {
        compose.onNodeWithText("Skills and agents").performScrollTo().performClick()
        compose.waitForIdle()
    }
    ...
    fun `a machine without the capability has no row`() {
        ...
        assertEquals(0, compose.onAllNodesWithText("Skills and agents").fetchSemanticsNodes().size)
    }

    @Test
    fun `the sheet lists skills, commands and subagents in sections with source, status and detail in words`() {
        show { MobileSkillsList("/work/alpha", listOf(alpha), listOf(deploy, review, fmt, reviewer)) }
        open()
    ...
    fun `a project with nothing says so`() {
        ...
        compose.onNodeWithText("This project has no skills, commands or subagents.").assertIsDisplayed()
    }
    ...
    fun `the lines are words and the empty sections are left out`() {
        ...
        assertEquals(listOf("Skills (2)", "Subagents (1)"), skillSections(listOf(deploy, review, reviewer)).map { it.first })
    }
