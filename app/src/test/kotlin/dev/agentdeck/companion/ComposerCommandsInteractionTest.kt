...
class ComposerCommandsInteractionTest {
    ...
    @Test
    fun `a Codex chat completes dollar skills and the machine's compact`() {
        // What a Codex chat is served: its skills, and `/compact`, which the machine runs as
        // Codex's own compaction. Before, `/` opened nothing there and `/compact` went as prose.
        answer = listOf(
            MobileCommand("/compact", "Summarize the conversation to free context"),
            MobileCommand("\$review", "Review a diff", skill = true),
        )
        ...
        compose.runOnIdle { assertEquals("/compact ", draft.value) }
    }
    ...
    @Test
    fun `render the Codex compact row`() {
        answer = listOf(
            MobileCommand("/compact", "Summarize the conversation to free context"),
            MobileCommand("\$review", "Review a diff", skill = true),
        )
        ...
        compose.onNode(hasSetTextAction()).performTextReplacement("/")
        compose.waitUntil(5_000) { loads > 0 }
        compose.onRoot().captureRoboImage("build/outputs/p15/codex-compact-popup.png", RECORD)
    }
