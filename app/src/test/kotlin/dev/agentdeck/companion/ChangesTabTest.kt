...
import dev.agentdeck.companion.ui.ListEntry
import dev.agentdeck.companion.ui.listEntries
...
import dev.agentdeck.companion.ui.copiedPath
...
class ChangesTabTest {
    ...
    private val views = mutableListOf<MobileReviewView>()
    private val sheets = mutableListOf<Pair<String, Set<String>?>>()
    ...
    @Test
    fun `under a request, commit and revert name its files and open on them alone, as the desk's file menu does`() {
        show(review = scoped, changesOpen = true, writable = true)
        compose.onNodeWithText("Commit…").performClick()
        compose.onNodeWithText("Revert…").performClick()

        compose.onNodeWithText("Whole session").performClick()
        compose.onNodeWithText("2. Document it").performClick()
        compose.onNodeWithText("Commit 2 files…").performClick()
        compose.onNodeWithText("Revert 2 files…").performClick()
        compose.onNodeWithText("2. Document it").performClick()
        compose.onNodeWithText("1. Move time behind a clock").performClick()
        compose.onNodeWithText("Commit 1 file…").performClick()

        val request2 = setOf("src/Clock.kt", "docs/time.md")
        assertEquals(
            listOf("commit" to null, "revert" to null, "commit" to request2, "revert" to request2, "commit" to setOf("src/Clock.kt")),
            sheets,
        )
    }
    ...
    /** `MainActivity` passes `onReattach` only under `review-note-reattach`; null must hide the button. */
    @Test
    fun `the notes sheet offers Reattach only where the machine can reattach`() {
        val note = com.github.claudeagents.core.mobile.MobileReviewNote("n1", "src/Clock.kt", "current", 2, 2, "x", "Why?", "open")
        val state = dev.agentdeck.companion.data.NotesState("k", notes = com.github.claudeagents.core.mobile.MobileReviewNotes("k", listOf(note)), listing = true)
        var reattach: ((String) -> Unit)? by androidx.compose.runtime.mutableStateOf(null)
        val picked = mutableListOf<String>()
        compose.setContent {
            AgentDeckTheme {
                dev.agentdeck.companion.ui.ReviewNotesSheetView(state, {}, {}, {}, {}, {}, {}, reattach, {}, {})
            }
        }
        compose.onAllNodes(androidx.compose.ui.test.hasTestTag("note-reattach")).assertCountEquals(0)

        reattach = { picked += it }
        compose.onNodeWithTag("note-reattach").performClick()
        assertEquals(listOf("n1"), picked)
    }
    ...
    /** The desk's "Group by directory" answer: blocks in the machine's order, each row naming its heading. */
    private val grouped = review.copy(
        files = listOf(
            MobileReviewFile("src/Clock.kt", MobileReviewFile.MODIFIED, added = 31, removed = 4, directory = "src"),
            MobileReviewFile("src/Time.kt", MobileReviewFile.MODIFIED, added = 2, removed = 1, directory = "src"),
            MobileReviewFile("docs/time.md", MobileReviewFile.MODIFIED, added = 6, removed = 2, reviewed = true, directory = "docs"),
            MobileReviewFile("README.md", MobileReviewFile.MODIFIED, added = 1, removed = 0, directory = ""),
        ),
        groupedByDirectory = true, groupingAdjustable = true, showFilePaths = false,
    )

    @Test
    fun `a grouped list paints the desk's directory headings, and a heading folds its block`() {
        show(changesOpen = true, review = grouped, viewable = true)
        compose.onNodeWithTag("changes-dir-src").assertIsDisplayed()
        compose.onNodeWithTag("changes-dir-").assertIsDisplayed()
        compose.onNodeWithContentDescription("Directory Project root, expanded, 1 file").assertIsDisplayed()
        // The heading already names the folder, so the row does not repeat it.
        compose.onAllNodesWithText("src").assertCountEquals(0)

        compose.onNodeWithTag("changes-dir-src").performClick()
        compose.onNodeWithText("Clock.kt").assertDoesNotExist()
        compose.onNodeWithContentDescription("Directory src, collapsed, 2 files").assertIsDisplayed()
        compose.onNodeWithText("time.md").assertIsDisplayed()
    ...
    @Test
    fun `the view menu writes the desk's toggles as their opposite of what is shown`() {
        show(changesOpen = true, review = grouped, viewable = true)
        compose.onNodeWithTag("changes-view-options").performClick()
        compose.onNodeWithTag("changes-group-by-directory").performClick()
        compose.onNodeWithTag("changes-view-options").performClick()
        compose.onNodeWithTag("changes-show-file-paths").performClick()

        assertEquals(listOf(MobileReviewView(groupByDirectory = false), MobileReviewView(showFilePaths = true)), views)
    }

    @Test
    fun `collapse and expand all fold every heading, offered only when there is more than one`() {
        show(changesOpen = true, review = grouped, viewable = true)
        compose.onNodeWithTag("changes-view-options").performClick()
        compose.onNodeWithTag("changes-expand-all").assertDoesNotExist()
        compose.onNodeWithTag("changes-collapse-all").performClick()
        compose.onNodeWithText("Clock.kt").assertDoesNotExist()
        compose.onNodeWithText("time.md").assertDoesNotExist()
        compose.onNodeWithContentDescription("Directory docs, collapsed, 1 file").assertIsDisplayed()

        compose.onNodeWithTag("changes-view-options").performClick()
        compose.onNodeWithTag("changes-collapse-all").assertDoesNotExist()
        compose.onNodeWithTag("changes-expand-all").performClick()
        compose.onNodeWithText("Clock.kt").assertIsDisplayed()
    }

    @Test
    fun `a ranked list offers no grouping, and a flat list's paths follow the desk`() {
        show(changesOpen = true, review = review.copy(showFilePaths = false), viewable = true)
        compose.onNodeWithText("src").assertDoesNotExist()
        compose.onNodeWithTag("changes-view-options").performClick()
        compose.onNodeWithTag("changes-group-by-directory").assertDoesNotExist()
        compose.onNodeWithTag("changes-collapse-all").assertDoesNotExist()
        compose.onNodeWithTag("changes-show-file-paths").performClick()
        assertEquals(listOf(MobileReviewView(showFilePaths = true)), views)
    }

    @Test
    fun `no view options against a machine that cannot write them, whose rows spell the folder`() {
        show(changesOpen = true, review = review, viewable = false)
        compose.onNodeWithTag("changes-view-options").assertDoesNotExist()
        compose.onNodeWithText("src").assertIsDisplayed()
    }

    @Test
    fun `a request's list takes its own order and shape from the desk's list of that request`() {
        // The session groups; request 2's own list is flat, spells paths, and puts docs first.
        val review = grouped.copy(
            requests = listOf(
                MobileReviewScope("p1", 1, "Clock", "Request 1 · 1 file", listOf("src/Clock.kt"), groupedByDirectory = true, showFilePaths = false),
                MobileReviewScope("p2", 2, "Docs", "Request 2 · 2 files", listOf("docs/time.md", "src/Time.kt"), showFilePaths = true),
            ),
        )
        show(changesOpen = true, review = review, viewable = true, diff = diff, follow = true)
        compose.onNodeWithText("Whole session").performClick()
        compose.onNodeWithText("2. Docs").performClick()

        compose.onNodeWithTag("changes-dir-src").assertDoesNotExist()
        compose.onNodeWithText("src").assertIsDisplayed()
        compose.onNodeWithText("time.md").performClick()
        compose.onNodeWithContentDescription("Next file").performClick()
        assertEquals("the stepper follows the request's order", listOf("docs/time.md", "src/Time.kt"), opened.map { it.first })
    }

    @Test
    fun `entries keep the machine's blocks and fold a directory to its heading`() {
        val entries = listEntries(grouped.files, grouped = true, collapsed = setOf("src"))
        assertEquals(
            listOf(ListEntry.Heading("src", true, 2), ListEntry.Heading("docs", false, 1), ListEntry.File(grouped.files[2]), ListEntry.Heading("", false, 1), ListEntry.File(grouped.files[3])),
            entries,
        )
        assertEquals(review.files.map { ListEntry.File(it) }, listEntries(review.files, grouped = false, collapsed = setOf("src")))
    }

    @Test
    fun `the grouping and its write survive the wire`() {
        assertEquals(grouped, MobileReviewList.fromJson(grouped.toJson()))
        assertEquals(null, MobileReviewList.fromJson(review.toJson()).showFilePaths)
        val sent = mutableListOf<String>()
        val client = BridgeClient(listOf("a.test"), 8443, "00", "token") { url ->
            object : HttpsURLConnection(url) {
                private val body = java.io.ByteArrayOutputStream()
                ...
                override fun getOutputStream(): java.io.OutputStream = body
                override fun getResponseCode(): Int = 200.also { sent += url.path + " " + body.toString() }
                override fun getInputStream(): InputStream = MobileReviewView(groupByDirectory = true).toJson().toString().byteInputStream()
                override fun getErrorStream(): InputStream = getInputStream()
            }
        }
        assertEquals(true, client.setReviewView(MobileReviewView(groupByDirectory = true)).groupByDirectory)
        assertEquals(true, sent.single().startsWith(MobileReviewView.ROUTE + " "))
        assertEquals(false, sent.single().contains("showFilePaths"))
    }
    ...
    @Test
    fun `an open diff copies the file's path on the machine, as the desk diff title does`() {
        val placed = review.copy(files = listOf(review.files.first().copy(absPath = "/work/app/src/Clock.kt")) + review.files.drop(1))
        show(review = placed, changesOpen = true, path = "src/Clock.kt", diff = diff.copy(file = placed.files.first()))

        compose.onNodeWithContentDescription("Copy path").performClick()
        val clipboard = ApplicationProvider.getApplicationContext<Context>().getSystemService(ClipboardManager::class.java)
        assertEquals("/work/app/src/Clock.kt", clipboard.primaryClip?.getItemAt(0)?.text?.toString())
    }

    @Test
    fun `a host without the machine path copies the conversation's own name for the file`() {
        assertEquals("src/Clock.kt", copiedPath("src/Clock.kt", review, diff = null))
        val placed = review.copy(files = listOf(review.files.first().copy(absPath = "/work/app/src/Clock.kt")))
        assertEquals("/work/app/src/Clock.kt", copiedPath("src/Clock.kt", placed, diff = null))
        assertEquals(placed.files.first(), MobileReviewFile.fromJson(placed.files.first().toJson()))
        assertEquals(null, MobileReviewFile.fromJson(review.files.first().toJson()).absPath)
    }
        ...
        compose.setContent {
            AgentDeckTheme {
                ConversationScreen(
                    ...
                    onReviewView = if (viewable) { view -> views.add(view) } else null,
                    onMarkReviewed = { paths, reviewed -> marks.add(paths to reviewed) },
                    onCommitChanges = if (writable) { scoped -> sheets.add("commit" to scoped) } else null,
                    onRevertChanges = if (writable) { scoped -> sheets.add("revert" to scoped) } else null,
                    reviewNotes = notes,
                )
            }
        }
