...
class PreviewInteractionTest {
    ...
    private var views = 0
    private val sames = mutableListOf<Long?>()
    private var rev = 0L
    ...
    private fun state(open: Boolean = true, unavailable: String? = null, canScroll: Boolean = true, canInteract: Boolean = false, canDrag: Boolean = canInteract,
        canAutoShow: Boolean = false, autoShow: Boolean = false,
    ) = MobilePreviewState(
        project = project.path, open = open, url = if (open) "http://localhost:3000/" else "", viewport = "PHONE", appearance = "SYSTEM",
        zoomPercent = 120, unavailable = unavailable, canScroll = canScroll, canInteract = canInteract, canDrag = canDrag,
        canAutoShow = canAutoShow, autoShow = autoShow,
    ...
    private fun picture(canScroll: Boolean = true, canInteract: Boolean = false, canDrag: Boolean = canInteract, canAutoShow: Boolean = false, autoShow: Boolean = false) =
        MobilePreviewView(state(canScroll = canScroll, canInteract = canInteract, canDrag = canDrag, canAutoShow = canAutoShow, autoShow = autoShow), MobilePreviewFrame(++rev, 390, 300, jpeg()))
    ...
    private fun show(
        ...
        refreshMs: Long = Long.MAX_VALUE,
    ) {
        val link = PreviewLink(
            projects = { projects },
            view = { _, _, same -> views++; sames += same; view() },
            act = { action -> sent += action; act(action) },
        )
        compose.setContent { AgentDeckTheme(dark = false, dynamic = false) { PreviewSheet(link, onDismiss = {}, refreshMs = refreshMs) } }
        compose.waitForIdle()
    ...
    @Test
    fun `the dev-server switch sends on then off, each read from the state the machine answered`() {
        var on = false
        show(
            view = { Result.success(picture(canAutoShow = true, autoShow = on)) },
            act = { action ->
                on = action.value == MobilePreviewAction.AUTO_SHOW_ON
                Result.success(MobilePreviewView(state(canAutoShow = true, autoShow = on)))
            },
        )

        compose.onNodeWithTag("preview-auto-show").performScrollTo().assertIsDisplayed().performClick()
        compose.waitForIdle()
        compose.onNodeWithTag("preview-dialog").captureRoboImage("build/outputs/p34/preview-auto-show.png", RECORD)
        compose.onNodeWithTag("preview-auto-show").performClick()
        compose.waitForIdle()

        assertEquals(
            listOf(MobilePreviewAction.AUTO_SHOW to "on", MobilePreviewAction.AUTO_SHOW to "off"),
            sent.map { it.action to it.value },
        )
        compose.onNodeWithTag("preview-picture").assertIsDisplayed()
    }
    ...
    @Test
    fun `the dev-server switch is there with no tab open, and absent from a machine that does not offer it`() {
        show(waitForPicture = false, view = { Result.success(MobilePreviewView(state(open = false, unavailable = "The Preview tab is not open on this machine. Open a page to start it.", canAutoShow = true))) })

        compose.onNodeWithTag("preview-auto-show").assertIsDisplayed()
        compose.onNodeWithText("Open Preview when a run starts a dev server").assertIsDisplayed()
        assertEquals(0, compose.onAllNodesWithContentDescription("Reload the page").fetchSemanticsNodes().size)
    }

    @Test
    fun `a machine that does not advertise the dev-server switch gets none`() {
        show(view = { Result.success(picture(canAutoShow = false)) })

        assertEquals(0, compose.onAllNodesWithTag("preview-auto-show").fetchSemanticsNodes().size)
        compose.onNodeWithContentDescription("Reload the page").assertIsDisplayed()
    }
    ...
    @Test
    fun `Drag on the page sends the start and end of the finger's path on the picture, and a press that never moved sends nothing`() {
        show(view = { Result.success(picture(canInteract = true)) }, act = { Result.success(picture(canInteract = true)) })

        compose.onNodeWithTag("preview-picture").performTouchInput { swipe(Offset(width / 4f, height / 2f), Offset(width * 3 / 4f, height / 2f)) }
        assertEquals("a drag on an unarmed picture reached the machine", 0, sent.size)

        compose.onNodeWithTag("preview-drag").performClick()
        compose.onNodeWithText("Drag on the picture. Turn Drag off to scroll.").assertIsDisplayed()
        compose.onNodeWithTag("preview-picture").performTouchInput { click(Offset(width / 2f, height / 2f)) }
        compose.waitForIdle()
        assertEquals("a tap is not a drag", 0, sent.size)
        compose.onNodeWithTag("preview-picture").performTouchInput { swipe(Offset(width / 4f, height / 2f), Offset(width * 3 / 4f, height / 4f)) }
        compose.waitForIdle()

        assertEquals(listOf(MobilePreviewAction.DRAG), sent.map { it.action })
        assertEquals(0.25, sent[0].x!!, 0.03)
        assertEquals(0.5, sent[0].y!!, 0.03)
        assertEquals(0.75, sent[0].toX!!, 0.03)
        assertEquals(0.25, sent[0].toY!!, 0.03)
        assertNotNull("a drag names the picture it was made on", sent[0].rev)
        compose.onNodeWithTag("preview-dialog").captureRoboImage("build/outputs/p34/preview-drag.png", RECORD)
    }

    @Test
    fun `a drag quotes the picture the finger went down on, not the one a poll brought while it travelled`() {
        var brought = 0L
        show(view = { Result.success(picture(canInteract = true)) }, act = { a -> Result.success(picture(canInteract = true).also { if (a.action == MobilePreviewAction.SCROLL) brought = it.frame!!.rev }) })
        compose.onNodeWithTag("preview-drag").performClick()

        compose.onNodeWithTag("preview-picture").performTouchInput { down(Offset(width / 4f, height / 2f)); moveTo(Offset(width * 3 / 4f, height / 4f)) }
        // A new picture arrives under the finger (the scroll's answer stands in for a poll's).
        compose.onNodeWithContentDescription("Scroll the page down").performSemanticsAction(SemanticsActions.OnClick)
        compose.waitForIdle()
        compose.onNodeWithTag("preview-picture").performTouchInput { up() }
        compose.waitForIdle()

        val drag = sent.single { it.action == MobilePreviewAction.DRAG }
        assertTrue("the drag named the picture that arrived under the finger (${drag.rev} of $brought), not the one it was drawn on", drag.rev!! < brought)
    }

    @Test
    fun `a machine that allows clicking but does not advertise dragging gets no drag control`() {
        show(view = { Result.success(picture(canInteract = true, canDrag = false)) })

        compose.onNodeWithTag("preview-click").assertIsDisplayed()
        assertEquals(0, compose.onAllNodesWithTag("preview-drag").fetchSemanticsNodes().size)
    }

    @Test
    fun `dragging, clicking and picking an element are one armed mode at a time`() {
        show(view = { Result.success(picture(canInteract = true)) }, act = { Result.success(picture(canInteract = true)) })
        fun tap() {
            compose.onNodeWithTag("preview-picture").performTouchInput { click(Offset(width / 2f, height / 2f)) }
            compose.waitForIdle()
        ...
        compose.onNodeWithTag("preview-click").performClick()
        tap()
        assertEquals("arming clicks must disarm dragging", listOf(MobilePreviewAction.CLICK), sent.map { it.action })

        sent.clear()
        compose.onNodeWithTag("preview-drag").performClick()
        tap()
        assertEquals("arming a drag must disarm clicking: a tap is not a drag", emptyList<String>(), sent.map { it.action })

        compose.onNodeWithContentDescription("Add an element to the chat").performClick()
        tap()
        assertEquals("picking an element must disarm dragging", listOf(MobilePreviewAction.ANNOTATE), sent.map { it.action })
    }
    ...
    @Test
    fun `a page that still paints the picture on screen confirms it, and that picture stays the one a tap quotes`() {
        var answers = 0
        show(refreshMs = 50, view = { if (answers++ == 0) Result.success(picture()) else Result.success(MobilePreviewView(state(), same = 1L)) })
        compose.waitUntil(5_000) { views >= 3 }

        compose.onNodeWithContentDescription("Add an element to the chat").performClick()
        compose.onNodeWithTag("preview-picture").performTouchInput { click(Offset(width / 2f, height / 2f)) }
        compose.waitForIdle()

        assertEquals("a poll names the picture on screen", 1L, sames.drop(1).first())
        assertEquals("the confirmed picture is not marked stale, so the tap reaches the machine", MobilePreviewAction.ANNOTATE, sent.single().action)
        assertEquals(1L, sent.single().rev)
    }

    @Test
    fun `an answer that names another picture confirms nothing, so the picture on screen is not tappable`() {
        var answers = 0
        show(refreshMs = 50, view = { if (answers++ == 0) Result.success(picture()) else Result.success(MobilePreviewView(state(), same = 99L)) })
        compose.waitUntil(5_000) { views >= 2 }

        compose.onNodeWithContentDescription("Add an element to the chat").performClick()
        compose.onNodeWithTag("preview-picture").performTouchInput { click(Offset(width / 2f, height / 2f)) }
        compose.waitForIdle()

        assertEquals("a tap on a picture the machine did not confirm reached it", 0, sent.size)
    }
    ...
    private fun settings(capable: Boolean) {
        ...
        val link = PreviewLink(projects = { Result.success(MobilePreviewProjects(listOf(project))) }, view = { _, _, _ -> Result.success(picture()) }, act = { Result.success(picture()) })
        compose.setContent {
