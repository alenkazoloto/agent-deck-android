...
class PreviewInteractionTest {
    ...
    private fun state(open: Boolean = true, unavailable: String? = null, canScroll: Boolean = true) = MobilePreviewState(
        project = project.path, open = open, url = if (open) "http://localhost:3000/" else "", viewport = "PHONE", appearance = "SYSTEM",
        zoomPercent = 120, unavailable = unavailable, canScroll = canScroll,
    ...
    private fun picture(canScroll: Boolean = true) = MobilePreviewView(state(canScroll = canScroll), MobilePreviewFrame(++rev, 390, 300, jpeg()))
