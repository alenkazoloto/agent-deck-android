...
class ScheduleIntoChatInteractionTest {
    ...
    private val waits = mutableListOf<List<MobileScheduleDependencySelection>>()
    private val sourceLoads = mutableListOf<String>()
    private val habits = mutableListOf<ScheduleHabit>()
    private val forms = mutableListOf<ScheduleChatForm>()
    ...
    /** mobile-todo Scheduling: the dialog opens on the last scheduling, as the desk's remembered "Run:" and Repeat do. */
    @Test
    fun `the dialog opens on the remembered cadence and a schedule records the one it used`() {
        show(habit = ScheduleHabit(ScheduleHabit.Due.IN, inAmount = 45, inMinutes = true, repeat = true))
        compose.onNode(hasSetTextAction()).performTextReplacement("Rerun the nightly suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()

        compose.onNodeWithTag("schedule-delay-amount").assertTextContains("45")
        compose.onNodeWithText("Repeat every 45 minutes").assertExists()
        val before = System.currentTimeMillis()
        compose.onNodeWithText("Schedule").performClick()

        compose.runOnIdle {
            val (due, repeat) = scheduled.single()
            assertTrue("due in 45 minutes: $due", due - before in (45 * 60_000L - 60_000)..(45 * 60_000L + 60_000))
            assertEquals(ScheduleRepeat(everyMs = 45 * 60_000L), repeat)
            assertEquals(ScheduleHabit(ScheduleHabit.Due.IN, inAmount = 45, inMinutes = true, repeat = true), habits.single())
        }
    }

    @Test
    fun `dismissing the dialog leaves the habit alone`() {
        show()
        compose.onNode(hasSetTextAction()).performTextReplacement("Rerun the nightly suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        compose.onNodeWithTag("schedule-into-chat-repeat").performClick()
        compose.onNodeWithText("Cancel").performClick()
        compose.runOnIdle { assertTrue(habits.isEmpty()) }
    }
    ...
    /** mobile-todo Scheduling row 2: the desk's "Schedule after sessions…", listed for the chat's own project. */
    @Test
    fun `a chat can wait for chosen sessions, which drops the repeat and posts them with their context`() {
        show(sources = sourcesOffer())
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        ...
        compose.onNodeWithTag("schedule-into-chat-repeat").assertDoesNotExist()
        compose.runOnIdle { assertEquals("the listing was read for another project", listOf("/project"), sourceLoads) }
        ...
        compose.runOnIdle {
            assertEquals(listOf(listOf(MobileScheduleDependencySelection("s1", inheritContext = true))), waits)
            assertEquals(listOf(false), afterRuns)
            assertEquals(null, scheduled.single().second)
        }
    }

    /** mobile-todo Scheduling form memory: a chat's own form wins over the machine's habit, as the desk's `FormDraft` does. */
    @Test
    fun `a chat that scheduled before opens on its own form, not the machine's habit`() {
        val own = ScheduleHabit(Due.IN, inAmount = 45, inMinutes = true, repeat = true)
        show(habit = ScheduleHabit(Due.AT, atTime = "07:45"), form = ScheduleChatForm(own))
        compose.onNode(hasSetTextAction()).performTextReplacement("Rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        ...
        compose.runOnIdle {
            val (due, repeat) = scheduled.single()
            assertTrue("due in 45 minutes: $due", due - before in (45 * 60_000L - 60_000)..(45 * 60_000L + 60_000))
            assertTrue(repeat != null)
            assertEquals("the machine's habit follows it, keeping its own typed At", listOf(own.copy(atTime = "07:45")), habits)
            assertEquals(listOf(ScheduleChatForm(own)), forms)
        }
    }

    @Test
    fun `cancelling keeps the chat's picks for next time and leaves the machine's habit`() {
        show(sources = sourcesOffer())
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        compose.onNodeWithText("In an hour").performClick()
        ...
        compose.runOnIdle {
            ...
            assertEquals(listOf(MobileScheduleDependencySelection("s1")), forms.single().dependencies)
        }
    }

    @Test
    fun `picks left unscheduled reopen on After sessions, without a session that has since gone`() {
        val left = listOf(MobileScheduleDependencySelection("s1", inheritContext = true), MobileScheduleDependencySelection("gone"))
        show(sources = sourcesOffer(), form = ScheduleChatForm(ScheduleHabit(), left))
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        ...
        compose.runOnIdle {
            assertEquals(listOf(listOf(MobileScheduleDependencySelection("s1", inheritContext = true))), waits)
            assertEquals("scheduling consumes the picks", listOf(emptyList<MobileScheduleDependencySelection>()), forms.map { it.dependencies })
        }
    }

    @Test
    fun `a machine that cannot hold a prompt for sessions does not offer the choice`() {
        show(sources = null)
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()

        compose.onNodeWithText("In an hour").performClick()
        compose.onNodeWithText("After sessions finish…").assertDoesNotExist()
    }

    @Test
    fun `a listing that failed says so and offers Retry`() {
        show(sources = ScheduleSourcesOffer(ScheduleSourcesState("/project", loading = false, error = "This IDE does not have that project open."), { sourceLoads += it }))
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        compose.onNodeWithText("In an hour").performClick()
        compose.onNodeWithText("After sessions finish…").performClick()

        compose.onNodeWithText("This IDE does not have that project open.").assertExists()
        compose.onNodeWithText("Retry").performClick()
        compose.runOnIdle { assertEquals(listOf("/project", "/project"), sourceLoads) }
        compose.onNodeWithText("Schedule").assertIsNotEnabled()
    }
    ...
    /** Not a golden: the dialog with "After sessions finish…" picked and one source chosen, under `build/outputs/schedule-into-chat/`. */
    @Test
    fun `render the dialog waiting for sessions`() {
        show(sources = sourcesOffer())
        compose.onNode(hasSetTextAction()).performTextReplacement("Then rerun the nightly suite")
        compose.onNodeWithContentDescription("Schedule this message").performClick()
        compose.onNodeWithText("In an hour").performClick()
        ...
        compose.waitForIdle()
        compose.onAllNodes(isRoot()).fetchSemanticsNodes().indices.last().let { last ->
            compose.onAllNodes(isRoot())[last].captureRoboImage("build/outputs/schedule-into-chat/dialog-after-sessions.png", RECORD)
        }
    }

    private fun sourcesOffer() = ScheduleSourcesOffer(
        ScheduleSourcesState("/project", loading = false, sources = listOf(
            MobileScheduleDependencySource("s1", "Build the fixture · pending schedule", "queued"),
            MobileScheduleDependencySource("s2", "Migrate the schema · claude / default", "idle"),
        )),
        onLoad = { sourceLoads += it },
    )

    private fun show(offer: Boolean = true, canAfterRun: Boolean = false, sources: ScheduleSourcesOffer? = null, habit: ScheduleHabit = ScheduleHabit(), form: ScheduleChatForm? = null) {
            ...
            CompositionLocalProvider(LocalNow provides { DeckFixtures.NOW }) { AgentDeckTheme {
                ConversationScreen(
                    ...
                    scheduleIntoChat = ScheduleIntoChatOffer(
                        ...
                        sources = sources,
                        onSchedule = { due, repeat, afterRun, dependencies ->
                            scheduled += due to repeat; afterRuns += afterRun; waits += dependencies
                        },
                        habit = habit,
                        onHabit = { habits += it },
                        form = form,
