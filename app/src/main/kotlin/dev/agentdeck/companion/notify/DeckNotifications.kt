...
object DeckNotifications {
    ...
    fun ensureChannels(context: Context) {
        ...
        NotifyTrigger.entries.forEach { trigger ->
            val importance = when (trigger) {
                NotifyTrigger.FINISHED -> NotificationManager.IMPORTANCE_LOW
                // A fuller window is news to plan around, not an agent waiting on the reader.
                NotifyTrigger.PLAN_USAGE -> NotificationManager.IMPORTANCE_DEFAULT
    ...
    /**
     * The desk's plan-usage warning, as one row that opens Settings › Usage.
     *
     * The id is the reset moment, so a second push about the same window — two IDEs on one
     * machine each send one — replaces the row instead of stacking it, and
     * [setOnlyAlertOnce] keeps that replacement quiet.
     */
    fun planUsage(context: Context, usage: MobilePush.PlanUsageAlert, machineId: String?) {
        if (!allowed(context)) return
        ensureChannels(context)
        val id = idOf("plan-usage:${usage.resetsAtMs}")
        val notification = NotificationCompat.Builder(context, NotifyTrigger.PLAN_USAGE.id)
            .setSmallIcon(R.drawable.ic_stat_deck)
            .setContentTitle(PlanUsageNotice.title(usage))
            .setContentText(PlanUsageNotice.body(usage))
            .setSubText(NotifyTrigger.PLAN_USAGE.title)
            .setAutoCancel(true)
            .setOnlyAlertOnce(true)
            .setCategory(NotificationCompat.CATEGORY_STATUS)
            .setContentIntent(activity(context, Navigation.link(Screen.Usage).orEmpty(), id, machineId))
            .build()
    ...
    private fun openIntent(context: Context, alert: Alert): PendingIntent {
        ...
        return activity(context, link, idOf(alert.key), alert.machineId)
    }

    private fun activity(context: Context, link: String, requestCode: Int, machineId: String? = null): PendingIntent {
        val intent = Intent(context, MainActivity::class.java).apply {
            action = Intent.ACTION_VIEW
            data = Uri.parse(link)
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
            machineId?.let { putExtra(MainActivity.EXTRA_MACHINE, it) }
        }
    ...
    private fun broadcast(
        ...
        val intent = Intent(context, NotifyReceiver::class.java).apply {
            this.action = action
            // A distinct data URI per (action, machine, conversation): PendingIntents are matched by
            ...
            data = Uri.parse("$action://${Uri.encode(alert.machineId.orEmpty())}/${Uri.encode(alert.key)}")
            ...
            alert.machineId?.let { putExtra(NotifyReceiver.EXTRA_MACHINE, it) }
        }
