...
data class Alert(
    ...
    /** The pairing whose fleet named [key]; the shade's Open, Reply and Stop go to it, not to whichever is active. */
...
object AttentionAlerts {
    ...
    fun plan(
        ...
        for (row in rows) {
            ...
            post += alert(row, trigger, machineId)
        }
    ...
    private fun alert(row: MobileFleetRow, trigger: NotifyTrigger, machineId: String?): Alert = Alert(
        key = row.key,
        trigger = trigger,
