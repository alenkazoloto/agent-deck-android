...
class NotifyReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        thread(name = "deck-notify-action") {
            try {
                ...
                // The machine whose fleet posted the row. The active pairing may have changed since,
                // and a reply sent there names a chat it does not have and parks in its drafts.
                val named = intent.getStringExtra(EXTRA_MACHINE)
                val machine = (if (named != null) store.machines().firstOrNull { it.id == named } else store.paired())
                    ?: run {
                        Log.i(TAG, "A shade action for $key names a machine this phone no longer pairs with")
                        return@thread
                    }
                ...
                when (intent.action) {
                    ...
                    ACTION_REPLY -> {
                        ...
                        runCatching {
                            client.send(replyRequest(key, project, vendor, reply))
                        }.onFailure { error ->
    ...
    companion object {
        /**
         * A shade reply names no pick — the composer is not on screen — so every field is left to
         * the desk: a machine advertising `send-desk-selection` reads the chat's own selectors as
         * the reply arrives, and an older one ignores the field and runs the CLI defaults as before.
         */
        fun replyRequest(key: String, project: String, vendor: AgentVendor, reply: String) = MobileSendRequest(
            key = key,
            projectPath = project,
            prompt = reply,
            vendor = vendor,
            ...
            permissionMode = null,
            newChat = false,
            deskFields = MobileRunSelection.Field.entries.toSet(),
        )
