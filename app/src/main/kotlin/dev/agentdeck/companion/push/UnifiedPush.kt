...
object UnifiedPush {
    ...
    /**
     * The application server's public key, base64url — UnifiedPush 3.x's `vapid` extra.
     *
     * The one extra whose value this app neither mints nor understands: it is the paired
     * machine's RFC 8292 identity, forwarded verbatim from `/v1/hello`. A 3.x distributor that
     * requires it answers [ACTION_REGISTRATION_FAILED] with [REASON_VAPID_REQUIRED] when it is
     * absent, and a 2.x one ignores it — so it is always sent when the machine has named one.
     */
    const val EXTRA_VAPID = "vapid"

    /** The distributor will not register an app that names no application server. */
     ...
     */
    fun register(context: Context, distributor: String, token: String, vapid: String? = null) {
        val intent = Intent(ACTION_REGISTER).apply {
            ...
            // Checked rather than forwarded blind: this string came off the network, the
            // distributor is a third-party app, and a malformed key is refused as *this*
            // app's registration — which the user experiences as push that will not turn on.
            if (MobilePush.isVapidPublicKey(vapid)) putExtra(EXTRA_VAPID, vapid)
