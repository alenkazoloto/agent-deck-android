...
object PushRegistration {
    ...
    fun show(context: Context, payload: MobilePush.Payload) {
        ...
        // The payload names no machine, so the active pairing is assumed, as the words already are.
        val machine = store.paired()
        // A plan window is the machine's, not a conversation's: nothing in the fleet cache names it.
        payload.usage?.let { return DeckNotifications.planUsage(context, it, machine?.id) }
        val snapshot = machine?.let { store.cachedSnapshot(it.id) }
        val alerts = payload.keys.map { key -> alert(key, trigger, snapshot, machine?.id) }
    ...
    private fun alert(key: String, trigger: NotifyTrigger, snapshot: MobileFleetSnapshot?, machineId: String?): Alert {
