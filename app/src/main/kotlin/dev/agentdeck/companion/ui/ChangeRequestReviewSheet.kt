 ...
 * the review body and the three verdicts; beside the title, the desk heading's Copy checkout command
 * and Re-read. A stacked request's neighbours and the changed-file
        ...
        Column(
            ...
            if (editing != null && actions.edits != null) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    review.checkoutCommand?.let { command ->
                        val clipboard = LocalClipboardManager.current
                        DeckIconButton(
                            label = "Copy the checkout command: $command",
                            icon = ImageVector.vectorResource(R.drawable.ic_content_copy),
                            onClick = { clipboard.setText(AnnotatedString(command)) },
                            modifier = Modifier.testTag("review-copy-checkout"),
                        )
                    }
                    DeckIconButton(
                        label = "Re-read the conversation",
                        icon = Icons.Filled.Refresh,
                        onClick = actions.onReload,
                        enabled = sheet.working == null && !sheet.reloading,
                        modifier = Modifier.testTag("review-reload"),
                    )
                }
                Text(
