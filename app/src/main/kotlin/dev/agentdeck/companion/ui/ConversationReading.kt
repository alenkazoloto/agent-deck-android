...
class ConversationReading internal constructor(initial: ReadingAnchor?, private val tolerance: Int) {
    ...
    /**
     * The top of what is loaded — item 0, not turn 0, so the "Load earlier messages" row comes
     * into view with it. A reader who jumped to the oldest message they have is exactly the
     * reader who wants the one above it, and following is dropped so the next frame does not
     * pull them straight back to the tail.
     */
    suspend fun oldest() {
        automatic = true
        following = false
