...
class ScheduleIntoChatOffer(
    ...
    val onSchedule: (dueAtMs: Long, repeat: ScheduleRepeat?, afterRun: Boolean) -> Unit,
)
...
internal fun ScheduleIntoChatDialog(
    ...
    val picked = due.picked(reset, offer.canAfterRun)
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                ScheduleDueSelector(due, hello, target, reset, runOffered = offer.canAfterRun)
                // A prompt that waits for the run has no cadence, so the desk stands the checkbox down; here it goes.
                if (offer.canRepeat && !waitsForRun) {
                    Row(
        ...
        confirmButton = {
            TextButton(
                ...
                onClick = {
                    onDismiss()
                    offer.onSchedule(picked.dueAtMs(), picked.repeat().takeIf { repeat && offer.canRepeat && !waitsForRun }, waitsForRun)
                },
            ) { Text("Schedule") }
        },
