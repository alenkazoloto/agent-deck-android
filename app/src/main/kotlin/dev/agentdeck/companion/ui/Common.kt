...
object Times {
    ...
    /** The words of a snackbar that confirms a schedule: [hostClock] against the machine [hello] named, now. */
    fun scheduledAt(dueAtMs: Long, hello: MobileHello?): String =
        hostClock(dueAtMs, System.currentTimeMillis(), hello?.timeZoneId?.let { runCatching { ZoneId.of(it) }.getOrNull() })
