...
fun RowSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = rememberModalBottomSheetState()) {
        Column(Modifier.fillMaxWidth().padding(bottom = 24.dp)) {
            ...
            RowActions.of(group, canReview, row, canOrganize, canShare, canDelete, canFolder, canFork).forEach { action ->
                when (action) {
                    ...
                    RowAction.FORK, RowAction.BRANCH -> SheetAction(painterResource(R.drawable.ic_fork), action.label) {
                        onDismiss()
                        onFork()
                    }
