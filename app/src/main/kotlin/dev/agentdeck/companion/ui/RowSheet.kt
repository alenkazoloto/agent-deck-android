...
fun RowSheet(
    ...
    onMcp: () -> Unit = {},
    /** The machine advertises `session-codex` (a phone holds the permission grant): a Codex chat's style, web-search mode and profile. */
    ...
    ModalBottomSheet(onDismissRequest = onDismiss, sheetState = rememberModalBottomSheetState()) {
        ...
        Column(Modifier.fillMaxWidth().verticalScroll(rememberScrollState()).padding(bottom = 24.dp)) {
            ...
            RowActions.of(
                ...
                canRewind = canRewind, canCommitStaged = canCommitStaged, canContext = canContext, canDirs = canDirs, canMcp = canMcp, canCodex = canCodex, canSpend = canSpend, canHandoff = canHandoff,
            ).forEach { action ->
                when (action) {
                    ...
                    RowAction.CODEX -> SheetAction(Icons.Filled.Settings, action.label) {
                        onDismiss()
                        onCodex()
                    }
                    ...
                    RowAction.COPY_ID -> SheetAction(painterResource(R.drawable.ic_content_copy), action.label) {
                        sessionIdOf(row.key)?.let { clipboard.setText(AnnotatedString(it)) }
                        onDismiss()
                    }
