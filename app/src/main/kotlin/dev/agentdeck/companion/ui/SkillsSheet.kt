 ...
 * for a name, scope, description and who may invoke it — never its script, model, effort or access. Editing
 * a skill's file, and installing a plugin stay on the desk. "Copy to project" / "Copy to personal" ([onCopy], null without `skill-copy`) is the desk's
 ...
 * desk's Uninstall on a marketplace plugin: a dialog first, then the same answer and re-read as a toggle. A "Marketplaces" section lists
...
@Composable
private fun SkillRow(row: MobileSkillRow, onToggle: (() -> Unit)?, onState: ((String) -> Unit)?, onUninstall: (() -> Unit)?, onCopy: (() -> Unit)?, onUpdate: (() -> Unit)?) {
    Column(Modifier.fillMaxWidth().testTag("skills-row").padding(top = 8.dp)) {
        Text(row.name, style = MaterialTheme.typography.bodyLarge)
        Detail(skillSourceLine(row))
        ...
        if (onToggle != null || onUninstall != null || onUpdate != null || onCopy != null) {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                if (onToggle != null) {
