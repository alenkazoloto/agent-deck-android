 ...
 *
 * Git, past-chat and symbol rows (`mention-context`) insert the desk's own tokens — `@diff`,
 * `@commit:7233791`, `@chat:1a2b3c4d`, `@symbol:MentionSession` — which the machine expands into
 * the diff, the conversation or the declaration's place on send; a branch row inserts the branch
 * name, as the desk's does. A peer row inserts another running session's name, which the agent
 * turns into a message to it, and an extension row inserts its `@<id>:<row>` token, which the
 * machine's extension expands on send. In a Codex chat a plugin row inserts its mention, which
 * Codex resolves itself.
 */
@Composable
fun MentionPopup(
...
private fun kindWord(kind: MobileMentionRow.Kind): String = when (kind) {
    ...
    MobileMentionRow.Kind.SYMBOL -> "Symbol"
    MobileMentionRow.Kind.PEER -> "Running session"
    MobileMentionRow.Kind.EXTENSION -> "Extension"
    MobileMentionRow.Kind.CODEX_PLUGIN -> "Codex plugin"
}
