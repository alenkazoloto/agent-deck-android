...
/**
 * "Another app handed you this text — where does it go?"
 *
 * There is no pick-a-thread screen behind it, and deliberately so: the fleet under this banner
 * *is* the picker, already filtered, sorted and holding the New-chat FAB. Tapping a row pastes
 * the share into that conversation's composer; ✕ throws it away. No second button for "new
 * chat" — the FAB is one thumb-width below and consumes the share the same way.
 */
@Composable
fun ShareBanner(shared: SharedInput?, onDismiss: () -> Unit) {
    if (shared == null) return
    Banner(
        "Sharing “${shared.label}” — tap a conversation, or start a new chat.",
        MaterialTheme.colorScheme.tertiaryContainer,
    ) {
        IconButton(onClick = onDismiss) {
            Icon(Icons.Filled.Close, contentDescription = "Discard shared text")
        }
    }
}
