...
object TranscriptQuestions {
    ...
    /**
     * Every question has at least one pick or, where the reader chose their own words, some.
     * [words] holds those: a key present with text is that question's whole answer.
     */
    fun complete(call: MobileToolCall, picks: Map<String, List<String>>, words: Map<String, String> = emptyMap()): Boolean =
        call.questions.all { picks[it.key].orEmpty().isNotEmpty() || words[it.key].orEmpty().isNotBlank() }
     ...
     * answer is its labels in the ask's own order, joined by ", " as the desktop form does. A
     * question the reader answered in [words] carries that text instead, trimmed and bounded.
     */
    fun answers(call: MobileToolCall, picks: Map<String, List<String>>, words: Map<String, String> = emptyMap()): Map<String, String> =
        call.questions.associate { question ->
            val typed = words[question.key].orEmpty().trim()
            if (typed.isNotEmpty()) question.key to typed.take(MobileAnswerRequest.MAX_TYPED_CHARS)
            else {
    ...
    /** The questions [answers] carries in the reader's words, which the host must not match to labels. */
    fun typedKeys(call: MobileToolCall, words: Map<String, String>): Set<String> =
        call.questions.map { it.key }.filterTo(LinkedHashSet()) { words[it].orEmpty().isNotBlank() }

    /** [picks] and [words] after tapping [label]: one or the other answers a question, never both. */
    fun pickLabel(
        picks: Map<String, List<String>>,
        words: Map<String, String>,
        question: MobileQuestion,
        label: String,
    ): Pair<Map<String, List<String>>, Map<String, String>> = toggle(picks, question, label) to (words - question.key)

    /** [picks] and [words] after opening "Something else" on [question]: its labels give way to the text. */
    fun openWords(
        picks: Map<String, List<String>>,
        words: Map<String, String>,
        question: MobileQuestion,
    ): Pair<Map<String, List<String>>, Map<String, String>> =
        (picks - question.key) to (words + (question.key to words[question.key].orEmpty()))
