 ...
 * menu (Photo, and File where the machine also advertises `attachment-pdf` — a PDF or text file, told apart by its bytes) instead of going straight to the photo picker; an older machine keeps the one-tap photo.
 */
@Composable
fun AttachButton(
    busy: Boolean,
    ...
    val fileLauncher = rememberLauncherForActivityResult(
        ...
        scope.launch {
            ...
            val (name, bytes) = withContext(Dispatchers.IO) {
                ...
                name to runCatching {
                    context.contentResolver.openInputStream(picked)?.use { readAtMost(it, (if (canAttachPdf) MobileAttachment.MAX_PDF_BYTES else MobileAttachment.MAX_TEXT_BYTES) + 1) }
                }.getOrNull()
            }
    ...
    Box(modifier) {
        ...
        DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }) {
            ...
            DropdownMenuItem(text = { Text(if (canAttachPdf) "File" else "Text file") }, onClick = { menuOpen = false; fileLauncher.launch(arrayOf("*/*")) })
        }
    }
}
