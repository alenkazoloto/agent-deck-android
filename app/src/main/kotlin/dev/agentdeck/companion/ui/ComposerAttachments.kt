 ...
 * menu (Photo, and File where the machine also advertises `attachment-pdf`, `attachment-zip`, `attachment-gzip`, `attachment-tar`, `attachment-xz`, `attachment-bzip2` or `attachment-zstd` — a PDF, archive, gzip, tar, xz, bzip2, Zstandard or text file, told apart by its bytes) instead of going straight to the photo picker; an older machine keeps the one-tap photo.
 */
@Composable
fun AttachButton(
    busy: Boolean,
    ...
    val fileLauncher = rememberLauncherForActivityResult(
        ...
        scope.launch {
            ...
            val (name, bytes) = withContext(Dispatchers.IO) {
                ...
                name to runCatching {
                    context.contentResolver.openInputStream(picked)?.use { readAtMost(it, (if (canAttachPdf || canAttachZip || canAttachGzip || canAttachTar || canAttachXz || canAttachBzip2 || canAttachZstd) MobileAttachment.MAX_PDF_BYTES else MobileAttachment.MAX_TEXT_BYTES) + 1) }
                }.getOrNull()
            }
    ...
    Box(modifier) {
        ...
        DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }) {
            ...
            DropdownMenuItem(text = { Text(if (canAttachPdf || canAttachZip || canAttachGzip || canAttachTar || canAttachXz || canAttachBzip2 || canAttachZstd) "File" else "Text file") }, onClick = { menuOpen = false; fileLauncher.launch(arrayOf("*/*")) })
        }
    }
}
