 ...
 * menu (Photo, and File where the machine also advertises `attachment-pdf`, `attachment-zip`, `attachment-gzip`, `attachment-tar`, `attachment-xz`, `attachment-bzip2`, `attachment-zstd`, `attachment-7z` or `attachment-rar` — a PDF, archive, gzip, tar, xz, bzip2, Zstandard, 7z, RAR or text file, told apart by its bytes) instead of going straight to the photo picker; an older machine keeps the one-tap photo.
 */
@Composable
fun AttachButton(
    busy: Boolean,
    ...
    /** `attachment-zstd`: likewise for a Zstandard file (a `.tar.zst` too), up to the same cap. */
    canAttachZstd: Boolean = false,
    /** `attachment-7z`: likewise for a 7z archive, up to the same cap. */
    canAttach7z: Boolean = false,
    /** `attachment-rar`: likewise for a RAR archive, up to the same cap. */
    ...
    val fileLauncher = rememberLauncherForActivityResult(
        ...
        scope.launch {
            ...
            val (name, bytes) = withContext(Dispatchers.IO) {
                ...
                name to runCatching {
                    context.contentResolver.openInputStream(picked)?.use { readAtMost(it, (if (canAttachPdf || canAttachZip || canAttachGzip || canAttachTar || canAttachXz || canAttachBzip2 || canAttachZstd || canAttach7z || canAttachRar) MobileAttachment.MAX_PDF_BYTES else MobileAttachment.MAX_TEXT_BYTES) + 1) }
                }.getOrNull()
            }
    ...
    Box(modifier) {
        ...
        DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }) {
            ...
            DropdownMenuItem(text = { Text(if (canAttachPdf || canAttachZip || canAttachGzip || canAttachTar || canAttachXz || canAttachBzip2 || canAttachZstd || canAttach7z || canAttachRar) "File" else "Text file") }, onClick = { menuOpen = false; fileLauncher.launch(arrayOf("*/*")) })
        }
    }
}
