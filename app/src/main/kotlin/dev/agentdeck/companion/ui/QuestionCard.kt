 ...
 *
 * [onAnswerTyped] is non-null only against a machine that advertises `answer-typed`, and is what
 * puts "Something else" under each question — the terminal's "Other". Against an older machine
 * the row would be a way to send words the host cannot tell from an invented label, which it
 * answers by starting a new turn, so the row is not offered at all.
 */
@Composable
internal fun QuestionCard(
    ...
    onAnswerTyped: ((answers: Map<String, String>, typed: Set<String>) -> Unit)? = null,
) {
    ...
    // A key present is a question answered in the reader's own words, even before they typed any.
    var words by rememberSaveable(call.id, stateSaver = wordsSaver) { mutableStateOf(emptyMap<String, String>()) }
    var sent by rememberSaveable(call.id, answerFailures) { mutableStateOf(false) }
    val open = live && !sent
    val submit = {
        sent = true
        val typed = TranscriptQuestions.typedKeys(call, words)
        val answers = TranscriptQuestions.answers(call, picks, words)
        if (typed.isEmpty() || onAnswerTyped == null) onAnswer(answers) else onAnswerTyped(answers, typed)
    }
    Column(Modifier.padding(top = 8.dp)) {
        call.questions.forEach { question ->
            ...
            question.options.forEach { option ->
                ...
                val pick = {
                    TranscriptQuestions.pickLabel(picks, words, question, option.label).let { (p, w) ->
                        picks = p
                        words = w
                    }
                    if (onTap) submit()
            ...
            if (onAnswerTyped != null) {
                val typing = question.key in words
                Surface(
                    Modifier
                        .fillMaxWidth()
                        .padding(bottom = 6.dp)
                        .then(
                            if (question.multiSelect) {
                                Modifier.toggleable(value = typing, enabled = open, role = Role.Checkbox) {
                                    TranscriptQuestions.openWords(picks, words, question).let { (p, w) -> picks = p; words = w }
                                }
                            } else {
                                Modifier.selectable(selected = typing, enabled = open, role = Role.RadioButton) {
                                    TranscriptQuestions.openWords(picks, words, question).let { (p, w) -> picks = p; words = w }
                                }
                            },
                        ),
                    shape = RoundedCornerShape(12.dp),
                    color = if (typing) MaterialTheme.colorScheme.primaryContainer
                    else MaterialTheme.colorScheme.surfaceContainerHighest,
                    contentColor = if (typing) MaterialTheme.colorScheme.onPrimaryContainer
                    else MaterialTheme.colorScheme.onSurface,
                    border = if (open) BorderStroke(1.dp, MaterialTheme.colorScheme.outlineVariant) else null,
                ) {
                    Text(
                ...
                if (typing && open) {
                    OutlinedTextField(
                        value = words[question.key].orEmpty(),
                        onValueChange = { text ->
                            words = words + (question.key to text.take(MobileAnswerRequest.MAX_TYPED_CHARS))
                        },
        ...
        if (open && (!onTap || words.isNotEmpty())) {
            Button(
                onClick = submit,
                enabled = TranscriptQuestions.complete(call, picks, words),
                modifier = Modifier.padding(bottom = 6.dp),
...
/** Question key and the words typed for it. */
private val wordsSaver = mapSaver<Map<String, String>>(
    save = { it },
    restore = { saved -> saved.mapValues { (_, text) -> text as String } },
)
