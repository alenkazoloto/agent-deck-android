...
fun NewChatScreen(
    ...
    /** "Clone repository…" and "Publish repository…"; null where the screen does not host their sheet. */
    repository: RepositoryButtons? = null,
) {
    val canAttach = MobileProtocol.Capability.ATTACHMENTS in hello?.capabilities.orEmpty()
    val canAttachFile = MobileProtocol.Capability.ATTACHMENT_TEXT in hello?.capabilities.orEmpty()
    val canAttachPdf = MobileProtocol.Capability.ATTACHMENT_PDF in hello?.capabilities.orEmpty()
    val canAttachZip = MobileProtocol.Capability.ATTACHMENT_ZIP in hello?.capabilities.orEmpty()
    val canAttachGzip = MobileProtocol.Capability.ATTACHMENT_GZIP in hello?.capabilities.orEmpty()
    val canAttachTar = MobileProtocol.Capability.ATTACHMENT_TAR in hello?.capabilities.orEmpty()
    val canAttachXz = MobileProtocol.Capability.ATTACHMENT_XZ in hello?.capabilities.orEmpty()
    val canAttachBzip2 = MobileProtocol.Capability.ATTACHMENT_BZIP2 in hello?.capabilities.orEmpty()
    val canAttachZstd = MobileProtocol.Capability.ATTACHMENT_ZSTD in hello?.capabilities.orEmpty()
    val canAttach7z = MobileProtocol.Capability.ATTACHMENT_7Z in hello?.capabilities.orEmpty()
    val canAttachRar = MobileProtocol.Capability.ATTACHMENT_RAR in hello?.capabilities.orEmpty()
    ...
    Column(Modifier.fillMaxSize()) {
        Column(
            ...
            OutlinedTextField(
                value = draft,
                onValueChange = { next ->
                    // A long paste leaves the field for a chip, as on the desk; at the per-message cap it stays inline.
                    val paste = onPastedText
                        ?.takeIf { canAttach && canAttachFile && photos.size < MobileAttachment.MAX_PER_SEND }
                        ?.let { PastedTextInsert.split(draft, next) }
                    if (paste == null) onDraft(next) else {
                        if (paste.kept != draft) onDraft(paste.kept)
            ...
            if (openProjects.isEmpty()) {
                ...
                FlowRow(
                    ...
                    PresetSelector(hello, target, onTarget)
                    AgentSelector(target, vendors, hello, onTarget)
                    // ACP has no account, model, effort, mode or toggles, so its start shows none of them.
                    ...
                    if (runPicks && ModelRows.of(hello, target.vendor, target.model).isNotEmpty()) {
                        ModelSelector(
                            hello = hello,
                            vendor = target.vendor,
                    ...
                    val capabilities = hello?.capabilities.orEmpty()
                    val canCreateWorktree = MobileProtocol.Capability.WORKTREE_CREATE in capabilities
                    // The rows come from the same read as "Manage worktrees…", so that capability is the gate.
                    val canPickWorktree = worktreeActions?.onPickExisting != null && MobileProtocol.Capability.WORKTREE_MANAGE in capabilities
                    if (worktreeActions != null && (canCreateWorktree || canPickWorktree)) {
                        Selector(
                            options = buildList {
                                add(SelectorOption("This checkout", CheckoutMode.HERE))
                                if (canCreateWorktree) add(SelectorOption("New worktree", CheckoutMode.NEW))
                                if (canPickWorktree) add(SelectorOption("Existing worktree", CheckoutMode.EXISTING))
                            },
                            selected = when {
                                worktree == null -> CheckoutMode.HERE
                                worktree.existing -> CheckoutMode.EXISTING
                                else -> CheckoutMode.NEW
                            },
                            onSelect = { mode ->
                                when (mode) {
                                    CheckoutMode.HERE -> worktreeActions.onPick(false, target.projectPath)
                                    CheckoutMode.NEW -> worktreeActions.onPick(true, target.projectPath)
                                    CheckoutMode.EXISTING -> worktreeActions.onPickExisting?.invoke(target.projectPath)
                                }
                            },
                            prefix = "Checkout:",
                        )
                    ...
                    if (repository != null && MobileProtocol.Capability.REPOSITORY_SETUP in hello?.capabilities.orEmpty()) {
                        // The desk offers Publish only where it would do something: a checkout with no remote.
                        LaunchedEffect(target.projectPath) { if (target.projectPath.isNotBlank()) repository.onCheck(target.projectPath) }
                        TextButton(onClick = { repository.onClone(target.projectPath) }, modifier = Modifier.heightIn(min = 48.dp).testTag("repository-clone")) {
                            Text("Clone repository…")
                        }
                        if (repository.publishable(target.projectPath)) {
                            TextButton(onClick = { repository.onPublish(target.projectPath) }, modifier = Modifier.heightIn(min = 48.dp).testTag("repository-publish")) {
        ...
        if (!crowded) Surface(tonalElevation = 1.dp, modifier = Modifier.liftsSnackbar()) {
                ...
                Button(
                    ...
                    Text(
                        when {
                            ...
                            sending -> "Starting…"
                            worktree?.existing == true -> "Start in worktree"
...
/** New chat's two repository rows: [publishable] is the machine's last answer to [onCheck]. */
class RepositoryButtons(
    val publishable: (projectPath: String) -> Boolean,
    val onCheck: (projectPath: String) -> Unit,
    val onClone: (projectPath: String) -> Unit,
    val onPublish: (projectPath: String) -> Unit,
)
...
class WorktreeActions(
    ...
    /** "Existing worktree" for this project; null where the screen has no worktree list to read. */
    val onPickExisting: ((projectPath: String) -> Unit)? = null,
    val onSelectExisting: (path: String) -> Unit = {},
...
private enum class CheckoutMode { HERE, NEW, EXISTING }

/** The project's own worktrees, one to start in; a listed directory that is gone is not offered. */
@Composable
private fun ExistingWorktrees(choice: WorktreeChoice, actions: WorktreeActions) {
    val rows = choice.rows
    if (rows.isEmpty()) {
        Text(
            "This project has no worktrees to start in. Choose New worktree to make one.",
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        return
    ...
    Selector(
        options = listOfNotNull(SelectorOption("Choose one", "").takeIf { choice.picked == null }) +
            rows.map { SelectorOption(it.label, it.path) },
        selected = choice.picked.orEmpty(),
        onSelect = actions.onSelectExisting,
        prefix = "Worktree:",
    )
}
...
private fun WorktreeFields(choice: WorktreeChoice, actions: WorktreeActions) {
    ...
    when {
        choice.existing && choice.loaded && refused == null -> ExistingWorktrees(choice, actions)
        !choice.loaded -> Text(
