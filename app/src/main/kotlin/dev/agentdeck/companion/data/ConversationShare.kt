...
object ConversationShare {
    ...
    fun write(root: File, export: MobileSessionExport, nowMs: Long): File {
        sweep(root, nowMs)
        ...
        return File(slot, safeName(export.fileName)).apply { writeText(export.text) }
    }

    /** The chooser for [file]; the recipient may read this one URI and nothing else. */
    fun chooser(context: Context, file: File, title: String): Intent {
        ...
        return Intent.createChooser(send, "Share conversation")
