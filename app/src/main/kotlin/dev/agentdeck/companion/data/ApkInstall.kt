...
object ApkInstall {
    ...
    /**
     * Every departure from this app goes through here, and the task flag is added *here* rather
     * than at each site.
     *
     * All three callers are handed the `Application` — the view model has no activity — and
     * Android refuses `startActivity` from a non-activity context without
     * [Intent.FLAG_ACTIVITY_NEW_TASK]. Two of the three carried it and [requestPermission] did
     * not, so the one screen the user was being sent to ("install unknown apps") threw on the
     * way out, was swallowed by the `runCatching` below, and the reader was left holding a
     * sentence telling them to turn on a setting with no route to it: the download landed, the
     * install was refused, and pressing Install again did the same nothing.
     */
