...
data class ReadingPositions(
    ...
    /** Per conversation, the send count its "where you left off" line was last offered at — see [RecapGate]. */
    val recaps: Map<String, Int> = emptyMap(),
) {
    fun recapOffered(key: String, userMessages: Int): ReadingPositions = copy(
        recaps = (recaps.filterKeys { it != key } + (key to userMessages)).entries.toList()
    ...
    fun toJson(): JsonObject = JsonObject().apply {
        ...
        add("recaps", JsonArray().also { rows ->
            recaps.entries.toList().takeLast(MAX_CONVERSATIONS).forEach { (key, at) ->
                rows.add(JsonObject().apply { addProperty("chat", key.take(MAX_KEY)); addProperty("at", at) })
            }
        })
        add("fleet", fleet.anchor.toJson().apply {
            ...
            add("filter", JsonObject().apply {
                ...
                addProperty("folder", fleet.filter.folderId?.take(MAX_KEY))
            })
        })
    }
    ...
    companion object {
        ...
        fun fromJson(json: JsonObject?): ReadingPositions = runCatching {
            ...
            val recaps = json.getAsJsonArray("recaps")?.toList().orEmpty().takeLast(MAX_CONVERSATIONS)
                .mapNotNull { element ->
                    val obj = element.takeIf { it.isJsonObject }?.asJsonObject ?: return@mapNotNull null
                    val key = obj.string("chat") ?: return@mapNotNull null
                    key to (runCatching { obj.get("at")?.asInt }.getOrNull() ?: return@mapNotNull null)
                }.toMap()
            ReadingPositions(chats, recaps = recaps, fleet = FleetBrowsing(
