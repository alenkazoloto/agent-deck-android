...
enum class RowAction(val label: String) {
    ...
    /** A Codex chat, whole: the desk's Branch chat (Codex's `thread/fork`). */
    BRANCH("Branch chat"),
...
object RowActions {
    ...
    fun of(
        ...
        if (canFork && row != null && group != FleetGroup.RUNNING) {
            add(if (row.vendor == com.github.claudeagents.core.AgentVendor.CODEX) RowAction.BRANCH else RowAction.FORK)
        }
