...
data class NewChatTarget(
    ...
    /** The preset the reader picked; it is *on* only while the cells still spell it — see [NewChat.presetFor]. */
    ...
    fun withVendor(next: AgentVendor): NewChatTarget = if (next == vendor && acpAgentId == null) this
        else copy(vendor = next, model = null, accountId = null, effort = null, permissionMode = null, fastMode = null, thinking = null, acpAgentId = null, presetName = null)

    fun withAcpAgent(id: String): NewChatTarget = if (id == acpAgentId) this
        else copy(vendor = AgentVendor.CLAUDE, model = null, accountId = null, effort = null, permissionMode = null, fastMode = null, thinking = null, acpAgentId = id, presetName = null)

    /** The desk's Apply: the preset's agent, account, model, effort and access, with its own Fast/Thinking left to the machine. */
    fun withPreset(preset: MobilePreset): NewChatTarget = copy(
        vendor = preset.vendor, model = preset.model.ifEmpty { null }, effort = preset.effort.ifEmpty { null },
        permissionMode = preset.permissionMode, accountId = preset.accountId,
        fastMode = null, thinking = null, acpAgentId = null, presetName = preset.name,
    )
...
object NewChat {
    ...
    /**
     * The presets to offer: none unless the machine advertised [MobileProtocol.Capability.AGENT_PRESETS], and none
     * for an ACP start, which has no cells to fill. A shipped row is offered on its own agent only; a saved one
     * may switch the agent, as on the desk.
     */
    fun presets(hello: MobileHello?, target: NewChatTarget): List<MobilePreset> =
        if (hello == null || MobileProtocol.Capability.AGENT_PRESETS !in hello.capabilities || acpAgentFor(hello, target) != null) emptyList()
        else hello.presets.filter { !it.builtIn || it.vendor == target.vendor }

    /**
     * The preset this target is on: the picked one, only while its cells still spell it. Moving a selector by hand
     * reads "No preset", as on the desk, and nothing is named on the wire — the machine would otherwise run
     * instructions for a bundle the reader has left. A preset that pins no account is on whichever account is picked.
     */
    fun presetFor(hello: MobileHello?, target: NewChatTarget): MobilePreset? {
        val name = target.presetName ?: return null
        val preset = presets(hello, target).firstOrNull { it.name == name } ?: return null
        val spelled = preset.vendor == target.vendor &&
            preset.model.ifEmpty { null } == target.model &&
            preset.effort.ifEmpty { null } == target.effort &&
            preset.permissionMode == target.permissionMode &&
            (preset.accountId == null || preset.accountId == target.accountId)
        return preset.takeIf { spelled }
    }
