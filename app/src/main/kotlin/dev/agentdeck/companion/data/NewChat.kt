...
data class NewChatTarget(
    ...
    fun withVendor(next: AgentVendor): NewChatTarget = if (next == vendor) this
        else copy(vendor = next, model = null, accountId = null, effort = null, permissionMode = null)

    val selection: MobileRunSelection get() = MobileRunSelection(model, effort, permissionMode)
}
...
object NewChat {
    ...
    fun accountFor(hello: MobileHello?, target: NewChatTarget): String? =
        accountFor(hello, target.vendor, target.accountId)
