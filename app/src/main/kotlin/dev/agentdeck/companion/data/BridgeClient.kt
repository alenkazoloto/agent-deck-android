...
class BridgeClient internal constructor(
    ...
    fun usage(agent: AgentVendor? = null, account: String? = null): MobileUsageReport {
        ...
        return MobileUsageReport.fromJson(get(MobileUsageReport.ROUTE + query))
    }
    ...
    /**
     * `POST /v1/attach` — one re-encoded JPEG, one UTF-8 text file, one PDF, one ZIP archive, one gzip file, one tar archive, one xz file, one bzip2 file, one Zstandard file or one 7z archive ([contentType]), each at its own cap.
     *
