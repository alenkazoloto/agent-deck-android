...
import com.github.claudeagents.core.mobile.MobilePromptSuggestions
...
import com.github.claudeagents.core.mobile.MobileRestrictedMode
import com.github.claudeagents.core.mobile.MobileContinueInterrupted
import com.github.claudeagents.core.mobile.MobileBranchMentions
import com.github.claudeagents.core.mobile.MobileContextWarning
import com.github.claudeagents.core.mobile.MobilePinNewChats
import com.github.claudeagents.core.mobile.MobileSkillReminder
import com.github.claudeagents.core.mobile.MobileSkillsInSlash
import com.github.claudeagents.core.mobile.MobileChatMarkup
import com.github.claudeagents.core.mobile.MobileJumpOnSend
import com.github.claudeagents.core.mobile.MobileQuickReplies
import com.github.claudeagents.core.mobile.MobileCacheClock
...
import com.github.claudeagents.core.mobile.MobileChatAutoCompact
import com.github.claudeagents.core.mobile.MobileQuestionsKeepWorking
import com.github.claudeagents.core.mobile.MobileAutoReview
import com.github.claudeagents.core.mobile.MobileAutoHandoff
import com.github.claudeagents.core.mobile.MobileAccountEdit
import com.github.claudeagents.core.mobile.MobileAccountEdited
...
import com.github.claudeagents.core.mobile.MobileExtensionsQuery
...
import com.github.claudeagents.core.mobile.MobileAcpAgentActed
import com.github.claudeagents.core.mobile.MobileAcpAgentQuery
...
import com.github.claudeagents.core.mobile.MobileMcpAdded
import com.github.claudeagents.core.mobile.MobileMcpRemove
import com.github.claudeagents.core.mobile.MobileMcpRemoved
...
import com.github.claudeagents.core.mobile.MobilePluginToggled
...
class BridgeClient internal constructor(
    ...
    /**
     * `GET /v1/tool/{key}/{callId}?thread=1` — the conversation a Task/Agent call ran, read-only. [owner] is the agent
     * whose own thread holds a call that delegated again, null for a call in the conversation itself.
     */
    fun subagentThread(key: String, callId: String, owner: String? = null): MobileSubagentThread = MobileSubagentThread.fromJson(
        get(
            "/v1/tool/" + URLEncoder.encode(key, "UTF-8") + "/" + URLEncoder.encode(callId, "UTF-8") +
                "?" + MobileSubagentThread.THREAD_PARAM + "=1" +
                (owner?.let { "&" + MobileSubagentThread.OWNER_PARAM + "=" + URLEncoder.encode(it, "UTF-8") } ?: ""),
        ),
    )
    ...
    /** `POST /v1/machine-behaviour` — writes the switch; answers the values now stored. */
    fun setPreventSleep(on: Boolean): MobileMachineBehaviour =
        MobileMachineBehaviour.fromJson(post(MobileMachineBehaviour.ROUTE, MobileMachineBehaviour.Change(preventSleep = on).toJson()))
            ?: throw java.io.IOException("The machine answered without its keep-awake setting.")

    /** `GET /v1/prompt-suggestions` — the desk's "Suggest the next prompt"; refused for a phone without the grant. */
    fun promptSuggestions(): MobilePromptSuggestions =
        MobilePromptSuggestions.fromJson(get(MobilePromptSuggestions.ROUTE))
            ?: throw java.io.IOException("The machine answered without its prompt-suggestions setting.")

    /** `POST /v1/prompt-suggestions` — writes the switch; answers the value now stored. */
    fun setPromptSuggestions(on: Boolean): MobilePromptSuggestions =
        MobilePromptSuggestions.fromJson(post(MobilePromptSuggestions.ROUTE, MobilePromptSuggestions(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its prompt-suggestions setting.")

    /** `GET /v1/restricted-mode` — the desk's "Restricted mode for Claude chats"; refused for a phone without the grant. */
    fun restrictedMode(): MobileRestrictedMode =
        MobileRestrictedMode.fromJson(get(MobileRestrictedMode.ROUTE))
            ?: throw java.io.IOException("The machine answered without its restricted-mode setting.")

    /** `POST /v1/restricted-mode` — writes the switch; answers the value now stored. */
    fun setRestrictedMode(on: Boolean): MobileRestrictedMode =
        MobileRestrictedMode.fromJson(post(MobileRestrictedMode.ROUTE, MobileRestrictedMode(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its restricted-mode setting.")

    /** `GET /v1/questions-keep-working` — the desk's "Let agents keep working while a question waits"; refused for a phone without the grant. */
    fun questionsKeepWorking(): MobileQuestionsKeepWorking =
        MobileQuestionsKeepWorking.fromJson(get(MobileQuestionsKeepWorking.ROUTE))
            ?: throw java.io.IOException("The machine answered without its question setting.")

    /** `POST /v1/questions-keep-working` — writes the switch; answers the value now stored. */
    fun setQuestionsKeepWorking(on: Boolean): MobileQuestionsKeepWorking =
        MobileQuestionsKeepWorking.fromJson(post(MobileQuestionsKeepWorking.ROUTE, MobileQuestionsKeepWorking(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its question setting.")

    /** `GET /v1/continue-interrupted` — the desk's "Continue an interrupted step automatically"; read by any pairing. */
    fun continueInterrupted(): MobileContinueInterrupted =
        MobileContinueInterrupted.fromJson(get(MobileContinueInterrupted.ROUTE))
            ?: throw java.io.IOException("The machine answered without its continuation setting.")

    /** `POST /v1/continue-interrupted` — writes the switch; answers the value now stored. */
    fun setContinueInterrupted(on: Boolean): MobileContinueInterrupted =
        MobileContinueInterrupted.fromJson(post(MobileContinueInterrupted.ROUTE, MobileContinueInterrupted(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its continuation setting.")

    /** `GET /v1/pin-new-chats` — the desk's "Pin a new chat to the top of the list once it starts"; read by any pairing. */
    fun pinNewChats(): MobilePinNewChats =
        MobilePinNewChats.fromJson(get(MobilePinNewChats.ROUTE))
            ?: throw java.io.IOException("The machine answered without its pinning setting.")

    /** `POST /v1/pin-new-chats` — writes the switch; answers the value now stored. */
    fun setPinNewChats(on: Boolean): MobilePinNewChats =
        MobilePinNewChats.fromJson(post(MobilePinNewChats.ROUTE, MobilePinNewChats(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its pinning setting.")

    /** `GET /v1/skill-reminder` — the desk's "Mention matching skills to Claude"; read by any pairing. */
    fun skillReminder(): MobileSkillReminder =
        MobileSkillReminder.fromJson(get(MobileSkillReminder.ROUTE))
            ?: throw java.io.IOException("The machine answered without its skill setting.")

    /** `POST /v1/skill-reminder` — writes the switch; answers the value now stored. */
    fun setSkillReminder(on: Boolean): MobileSkillReminder =
        MobileSkillReminder.fromJson(post(MobileSkillReminder.ROUTE, MobileSkillReminder(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its skill setting.")

    /** `GET /v1/skills-in-slash` — the desk's "Show skills in the slash menu"; read by any pairing. */
    fun skillsInSlash(): MobileSkillsInSlash =
        MobileSkillsInSlash.fromJson(get(MobileSkillsInSlash.ROUTE))
            ?: throw java.io.IOException("The machine answered without its slash-menu setting.")

    /** `POST /v1/skills-in-slash` — writes the switch; answers the value now stored. */
    fun setSkillsInSlash(on: Boolean): MobileSkillsInSlash =
        MobileSkillsInSlash.fromJson(post(MobileSkillsInSlash.ROUTE, MobileSkillsInSlash(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its slash-menu setting.")

    /** `GET /v1/chat-markup` — the desk's "Render formatting in messages"; read by any pairing. */
    fun chatMarkup(): MobileChatMarkup =
        MobileChatMarkup.fromJson(get(MobileChatMarkup.ROUTE))
            ?: throw java.io.IOException("The machine answered without its formatting setting.")

    /** `POST /v1/chat-markup` — writes the switch; answers the value now stored. */
    fun setChatMarkup(on: Boolean): MobileChatMarkup =
        MobileChatMarkup.fromJson(post(MobileChatMarkup.ROUTE, MobileChatMarkup(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its formatting setting.")

    /** `GET /v1/jump-on-send` — the desk's "Scroll to the newest message when I send"; read by any pairing. */
    fun jumpOnSend(): MobileJumpOnSend =
        MobileJumpOnSend.fromJson(get(MobileJumpOnSend.ROUTE))
            ?: throw java.io.IOException("The machine answered without its scroll setting.")

    /** `POST /v1/jump-on-send` — writes the switch; answers the value now stored. */
    fun setJumpOnSend(on: Boolean): MobileJumpOnSend =
        MobileJumpOnSend.fromJson(post(MobileJumpOnSend.ROUTE, MobileJumpOnSend(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its scroll setting.")

    /** `GET /v1/branch-mentions` — the desk's "Complete git branches after @"; read by any pairing. */
    fun branchMentions(): MobileBranchMentions =
        MobileBranchMentions.fromJson(get(MobileBranchMentions.ROUTE))
            ?: throw java.io.IOException("The machine answered without its branch-completion setting.")

    /** `POST /v1/branch-mentions` — writes the switch; answers the value now stored. */
    fun setBranchMentions(on: Boolean): MobileBranchMentions =
        MobileBranchMentions.fromJson(post(MobileBranchMentions.ROUTE, MobileBranchMentions(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its branch-completion setting.")

    /** `GET /v1/context-warning` — the desk's "Warn when a chat context is nearly full"; read by any pairing. */
    fun contextWarning(): MobileContextWarning =
        MobileContextWarning.fromJson(get(MobileContextWarning.ROUTE))
            ?: throw java.io.IOException("The machine answered without its context-warning setting.")

    /** `POST /v1/context-warning` — writes the switch; answers the value now stored. */
    fun setContextWarning(on: Boolean): MobileContextWarning =
        MobileContextWarning.fromJson(post(MobileContextWarning.ROUTE, MobileContextWarning(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its context-warning setting.")

    /** `GET /v1/quick-replies` — the desk's "Show quick replies"; read by any pairing. */
    fun quickReplies(): MobileQuickReplies =
        MobileQuickReplies.fromJson(get(MobileQuickReplies.ROUTE))
            ?: throw java.io.IOException("The machine answered without its quick-replies setting.")

    /** `POST /v1/quick-replies` — writes the switch; answers the value now stored. */
    fun setQuickReplies(on: Boolean): MobileQuickReplies =
        MobileQuickReplies.fromJson(post(MobileQuickReplies.ROUTE, MobileQuickReplies(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its quick-replies setting.")

    /** `GET /v1/pinned-todos` — the desk's "Pin the agent's todo list"; read by any pairing. */
    fun pinnedTodos(): MobilePinnedTodos =
        MobilePinnedTodos.fromJson(get(MobilePinnedTodos.ROUTE))
            ?: throw java.io.IOException("The machine answered without its pinned-todo-list setting.")

    /** `POST /v1/pinned-todos` — writes the switch; answers the value now stored. */
    fun setPinnedTodos(on: Boolean): MobilePinnedTodos =
        MobilePinnedTodos.fromJson(post(MobilePinnedTodos.ROUTE, MobilePinnedTodos(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its pinned-todo-list setting.")

    /** `GET /v1/cache-clock` — the desk's "Show prompt cache clock"; read by any pairing. */
    fun cacheClock(): MobileCacheClock =
        MobileCacheClock.fromJson(get(MobileCacheClock.ROUTE))
            ?: throw java.io.IOException("The machine answered without its cache-clock setting.")

    /** `POST /v1/cache-clock` — writes the switch; answers the value now stored. */
    fun setCacheClock(on: Boolean): MobileCacheClock =
        MobileCacheClock.fromJson(post(MobileCacheClock.ROUTE, MobileCacheClock(on).toJson()))
            ?: throw java.io.IOException("The machine answered without its cache-clock setting.")

    /** `GET /v1/chat-auto-hide` — the desk's "Hide chats I haven't opened in N days"; read by any pairing. */
    fun chatAutoHide(): MobileChatAutoHide =
        MobileChatAutoHide.fromJson(get(MobileChatAutoHide.ROUTE))
            ?: throw java.io.IOException("The machine answered without its auto-hide setting.")

    /** `POST /v1/chat-auto-hide` — writes the number of days; answers the value now stored. */
    fun setChatAutoHide(days: Int): MobileChatAutoHide =
        MobileChatAutoHide.fromJson(post(MobileChatAutoHide.ROUTE, MobileChatAutoHide(days).toJson()))
            ?: throw java.io.IOException("The machine answered without its auto-hide setting.")

    /** `GET /v1/chat-auto-compact` — the desk's "Compact chats idle for N days"; refused for a phone without the grant. */
    fun chatAutoCompact(): MobileChatAutoCompact =
        MobileChatAutoCompact.fromJson(get(MobileChatAutoCompact.ROUTE))
            ?: throw java.io.IOException("The machine answered without its auto-compact setting.")

    /** `POST /v1/chat-auto-compact` — writes the number of days; answers the value now stored. */
    fun setChatAutoCompact(days: Int): MobileChatAutoCompact =
        MobileChatAutoCompact.fromJson(post(MobileChatAutoCompact.ROUTE, MobileChatAutoCompact(days).toJson()))
            ?: throw java.io.IOException("The machine answered without its auto-compact setting.")

    /** `GET /v1/auto-handoff` — the desk's "Continue on another account near the usage limit"; refused for a phone without the grant. */
    fun autoHandoff(): MobileAutoHandoff =
        MobileAutoHandoff.fromJson(get(MobileAutoHandoff.ROUTE))
            ?: throw java.io.IOException("The machine answered without its account-handoff settings.")

    /** `POST /v1/auto-handoff` — edits the fields [change] names; answers all three values now stored. */
    fun setAutoHandoff(change: MobileAutoHandoff.Change): MobileAutoHandoff =
        MobileAutoHandoff.fromJson(post(MobileAutoHandoff.ROUTE, change.toJson()))
            ?: throw java.io.IOException("The machine answered without its account-handoff settings.")

    /** `GET /v1/auto-review` — the desk's two "Review changes automatically" switches; refused for a phone without the grant. */
    fun autoReview(): MobileAutoReview =
        MobileAutoReview.fromJson(get(MobileAutoReview.ROUTE))
            ?: throw java.io.IOException("The machine answered without its automatic-review settings.")

    /** `POST /v1/auto-review` — flips the switches [change] names; answers both values now stored. */
    fun setAutoReview(change: MobileAutoReview.Change): MobileAutoReview =
        MobileAutoReview.fromJson(post(MobileAutoReview.ROUTE, change.toJson()))
            ?: throw java.io.IOException("The machine answered without its automatic-review settings.")

    /** `POST /v1/machine-behaviour` — writes "Also while a session waits for my review" alone. */
    fun setHoldForReview(on: Boolean): MobileMachineBehaviour =
        MobileMachineBehaviour.fromJson(post(MobileMachineBehaviour.ROUTE, MobileMachineBehaviour.Change(holdForReview = on).toJson()))
            ?: throw java.io.IOException("The machine answered without its keep-awake setting.")
    ...
    /** `GET /v1/codex-output` — the desk's Codex answer length and reasoning summary and the words it offers; read by any pairing. */
    fun codexOutput(): MobileCodexOutput =
        MobileCodexOutput.fromJson(get(MobileCodexOutput.ROUTE))
            ?: throw java.io.IOException("The machine answered without its Codex output settings.")

    /** `POST /v1/codex-output` — sets the cell or cells named by [change]; answers the pair now stored. */
    fun setCodexOutput(change: MobileCodexOutput.Change): MobileCodexOutput =
        MobileCodexOutput.fromJson(post(MobileCodexOutput.ROUTE, change.toJson()))
            ?: throw java.io.IOException("The machine answered without its Codex output settings.")
    ...
    /** `POST /v1/account-edit` — the desk's Accounts › Rename / Remove; answers the name now stored, none once the account is gone. */
    fun editAccount(edit: MobileAccountEdit): MobileAccountEdited =
        MobileAccountEdited.fromJson(post(MobileAccountEdit.ROUTE, edit.toJson()))
            ?: throw java.io.IOException("The machine answered without the account.")
    ...
    fun setScheduleOverlap(allow: Boolean): MobileScheduleSettings =
        MobileScheduleSettings.fromJson(post(MobileScheduleSettings.ROUTE, MobileScheduleSettings(allowOverlap = allow).toJson()))
            ?: throw java.io.IOException("The machine answered without its overlapping-runs setting.")

    /** `POST /v1/schedule-settings` — the background-run count; answers the value now stored. */
    fun setScheduleBackgroundRuns(count: Int): MobileScheduleSettings =
        MobileScheduleSettings.fromJson(post(MobileScheduleSettings.ROUTE, MobileScheduleSettings(backgroundRuns = count).toJson()))
            ?: throw java.io.IOException("The machine answered without its background-runs setting.")
    ...
    /** `GET /v1/session-codex?key=` — the Codex chat's three controls; refused with `permissions-disabled` for a phone without the grant. */
    fun sessionCodex(key: String): MobileSessionCodex = MobileSessionCodex.fromJson(
        get(MobileSessionCodexQuery.ROUTE + "?key=" + URLEncoder.encode(key, "UTF-8")),
    )

    /** `POST /v1/session-codex` — one control's value; a value the machine does not list comes back as [MobileSessionCodex.refused], not an error. */
    fun changeSessionCodex(request: MobileSessionCodexRequest): MobileSessionCodex =
        MobileSessionCodex.fromJson(post(MobileSessionCodexQuery.ROUTE, request.toJson()))
    ...
    /**
     * `GET /v1/insights` — the desk's Session insights over the spend cut the Usage screen shows under this Agent / Account
     * pick. Read timeout as the export's: the machine walks its whole index.
     */
    fun insights(agent: AgentVendor? = null, account: String? = null): MobileInsights = MobileInsights.fromJson(
        get(MobileInsights.ROUTE + usageQuery(agent, account), readTimeoutMs = LinkPolicy.EXPORT_READ_TIMEOUT_MS, retry = { it !is java.net.SocketTimeoutException }),
    )
    ...
    /** `GET /v1/preview` — the open projects whose Preview tab this phone may look at; refused without the Preview grant. */
    fun previewProjects(): MobilePreviewProjects = MobilePreviewProjects.fromJson(get(MobilePreviewAction.ROUTE))

    /**
     * `GET /v1/preview?project=[&frame=1[&same=rev]]` — the desk's Preview state and, when asked, one picture of the page. [same] names the picture on
     * screen: a page that still paints it answers with that rev and no picture.
     */
    fun preview(project: String, frame: Boolean, same: Long? = null): MobilePreviewView =
        MobilePreviewView.fromJson(
            get(MobilePreviewAction.ROUTE + "?project=" + URLEncoder.encode(project, "UTF-8") + if (frame) "&frame=1" + (same?.let { "&same=$it" } ?: "") else ""),
        )
            ?: throw java.io.IOException("The machine answered without its preview.")

    /** `POST /v1/preview` — one intent; answered with the state, a fresh picture and a sentence about what happened. */
    fun previewAct(action: MobilePreviewAction): MobilePreviewView =
        MobilePreviewView.fromJson(post(MobilePreviewAction.ROUTE, action.toJson()))
            ?: throw java.io.IOException("The machine answered without its preview.")
    ...
    /** `GET /v1/extensions` — Resources › Extensions' installed list, read-only. */
    fun extensions(): MobileExtensions = MobileExtensions.fromJson(get(MobileExtensionsQuery.ROUTE))
    ...
    /** `GET /v1/mcp-health[?project=]` — the desk's "Check connections": Claude's servers' state words, the machine connecting only because this was asked. */
    fun mcpHealth(project: String?): MobileMcpHealth = MobileMcpHealth.fromJson(
        get(MobileMcpQuery.HEALTH_ROUTE + (project?.let { "?project=" + URLEncoder.encode(it, "UTF-8") } ?: "")),
    )

    /** `POST /v1/mcp-remove` — the desk's MCP Remove for one listed server; the answer's failure is the machine's sentence, null when it is gone. */
    fun removeMcpServer(remove: MobileMcpRemove): MobileMcpRemoved =
        MobileMcpRemoved.fromJson(post(MobileMcpQuery.REMOVE_ROUTE, remove.toJson()))

    /** `POST /v1/mcp-add` — the desk's MCP add for one remote server; the answer's failure is the machine's sentence, null when it is added. */
    fun addMcpServer(add: MobileMcpAdd): MobileMcpAdded =
        MobileMcpAdded.fromJson(post(MobileMcpQuery.ADD_ROUTE, add.toJson()))

    /** `GET /v1/acp-agents` — each switched-on ACP agent's connection state, the desk's advice and the sign-in methods it offered; starts nothing. */
    fun acpAgents(): MobileAcpAgents = MobileAcpAgents.fromJson(get(MobileAcpAgentQuery.ROUTE))

    /** `POST /v1/acp-agent-action` — the desk's Connect or Sign in; a null failure means started, and the next [acpAgents] says how it ended. */
    fun actOnAcpAgent(action: MobileAcpAgentAction): MobileAcpAgentActed =
        MobileAcpAgentActed.fromJson(post(MobileAcpAgentQuery.ACTION_ROUTE, action.toJson()))

    /** `POST /v1/plugin-toggle` — the desk's Plugins Enable / Disable for one listed plugin; the answer's failure is the machine's sentence, null when it is done. */
    fun togglePlugin(toggle: MobilePluginToggle): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.TOGGLE_ROUTE, toggle.toJson()))

    /** `POST /v1/skill-state` — the desk's Skills state combo for one listed skill or command; the answer is a toggle's. */
    fun setSkillState(state: com.github.claudeagents.core.mobile.MobileSkillState): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.STATE_ROUTE, state.toJson()))

    /** `POST /v1/skill-copy` — the desk's Skills "Copy to project" / "Copy to personal" for one listed skill; the answer is a toggle's. */
    fun copySkill(copy: com.github.claudeagents.core.mobile.MobileSkillCopy): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.COPY_ROUTE, copy.toJson()))

    /** `POST /v1/marketplace-refresh` — the desk's Marketplaces update for one listed plugin marketplace; the answer is a toggle's. */
    fun refreshMarketplace(change: MobileMarketplaceChange): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.MARKETPLACE_REFRESH_ROUTE, change.toJson()))

    /** `POST /v1/marketplace-remove` — the desk's Marketplaces Remove for one listed plugin marketplace; the answer is a toggle's. */
    fun removeMarketplace(change: MobileMarketplaceChange): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.MARKETPLACE_REMOVE_ROUTE, change.toJson()))

    /** `POST /v1/skill-create` — the desk's Skills "New Skill…" for a name, scope, description and invocation; the answer is a toggle's. */
    fun createSkill(create: com.github.claudeagents.core.mobile.MobileSkillCreate): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.CREATE_ROUTE, create.toJson()))

    /**
     * `GET /v1/skill-file?project=&name=&kind=&source=` — one listed skill's or command's own file: its text, the revision a save must
     * quote and whether this phone may save. The file is named by the row, never a path; the machine says why it is not offered.
     */
    fun skillFile(project: String, name: String, kind: String, source: String): MobileMemoryFile = MobileMemoryFile.fromJson(
        get(
            MobileSkillsQuery.FILE_ROUTE + "?project=" + URLEncoder.encode(project, "UTF-8") + "&name=" + URLEncoder.encode(name, "UTF-8") +
                "&kind=" + URLEncoder.encode(kind, "UTF-8") + if (source.isEmpty()) "" else "&source=" + URLEncoder.encode(source, "UTF-8"),
        ),
    )

    /** `POST /v1/skill-file` — saves the whole text against [MobileSkillFileSave.revision]; a changed file is a conflict answer, not an error. */
    fun saveSkillFile(request: com.github.claudeagents.core.mobile.MobileSkillFileSave): MobileMemorySaved =
        MobileMemorySaved.fromJson(post(MobileSkillsQuery.FILE_ROUTE, request.toJson()))

    /** `POST /v1/plugin-update` — the desk's Plugins Update for one listed marketplace plugin; the answer is a toggle's. */
    fun updatePlugin(update: MobilePluginUpdate): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.UPDATE_ROUTE, update.toJson()))

    /** `POST /v1/plugin-install` — the desk's Plugin Catalog Install for one plugin a marketplace lists; the answer is a toggle's. */
    fun installPlugin(install: MobilePluginInstall): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.INSTALL_ROUTE, install.toJson()))

    /** `POST /v1/plugin-uninstall` — the desk's Plugins Uninstall for one listed marketplace plugin; the answer is a toggle's. */
    fun uninstallPlugin(uninstall: MobilePluginUninstall): MobilePluginToggled =
        MobilePluginToggled.fromJson(post(MobileSkillsQuery.UNINSTALL_ROUTE, uninstall.toJson()))
    ...
    /** `GET /v1/memory?project=&loadOrder=1` — the desk's Load order for one open project: its steps in order, and the desk's summary. */
    fun memoryLoadOrder(project: String): MobileMemoryLoadOrder = MobileMemoryLoadOrder.fromJson(
        get(MobileMemoryQuery.ROUTE + "?project=" + URLEncoder.encode(project, "UTF-8") + "&" + MobileMemoryQuery.LOAD_ORDER_PARAM + "=1"),
    )
    ...
    /** `GET /v1/prompt-library` — the desk's saved prompts (`/prompts`), newest first. */
    fun promptLibrary(): MobilePromptLibrary = MobilePromptLibrary.fromJson(get(MobilePromptLibrary.ROUTE))

    /** `POST /v1/prompt-library` — keeps one prompt (`/saveprompt`); answers the list now stored. */
    fun savePrompt(name: String, text: String): MobilePromptLibrary =
        MobilePromptLibrary.fromJson(post(MobilePromptLibrary.ROUTE, MobilePromptLibrary.Save(name, text).toJson()))
    ...
    /**
     * Stops one live background task of a chat the desk is streaming. The CLI's own refusal comes back
     * as [MobileTaskStopped.refused] with the task still running; `task-not-stoppable` is a refusal
     * of the call, for a run that has no channel to stop it on.
     */
    fun stopTask(request: MobileTaskStopRequest): MobileTaskStopped =
        MobileTaskStopped.fromJson(post(MobileTaskStopQuery.ROUTE, request.toJson()))
