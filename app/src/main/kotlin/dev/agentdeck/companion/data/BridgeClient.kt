...
class BridgeClient internal constructor(
    ...
    /**
     * `POST /v1/attach` — one re-encoded JPEG or one UTF-8 text file ([contentType]), each at its own cap.
     *
     ...
     */
    fun attach(bytes: ByteArray, contentType: String = MobileAttachment.CONTENT_TYPE): MobileAttachmentAccepted = walkHosts { host ->
        val cap = if (MobileAttachment.isTextContentType(contentType)) MobileAttachment.MAX_TEXT_BYTES else MobileAttachment.MAX_ATTACH_BYTES
        if (bytes.size > cap) {
            throw BridgeRefusal(
