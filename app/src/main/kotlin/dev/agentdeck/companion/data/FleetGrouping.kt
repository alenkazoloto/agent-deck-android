...
object FleetGrouping {
    ...
    /**
     * The desk's pins lead every ordering in the desk's own pinned order, as they lead its lists
     * (`ConversationShelf.pinnedFirst`). A pin without a rank — an older plugin's — ties with the
     * others and falls to the ordering below, as every pin did before.
     */
    private val pinnedFirst: Comparator<MobileFleetRow> =
        compareBy<MobileFleetRow> { !it.pinned }.thenBy { if (it.pinned) it.pinRank ?: Int.MAX_VALUE else 0 }
