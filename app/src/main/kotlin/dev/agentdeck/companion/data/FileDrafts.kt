...
internal class FileDrafts {
    ...
    fun opened(key: String, file: MobileMemoryFile): OpenMemoryFile {
        ...
        val fresh = OpenMemoryFile(
            file.id, file.label, file.writable, file.deletable, file.existed, file.content, file.revision, file.content,
            cap = file.cap, rulePaths = file.rulePaths,
        )
    ...
    fun applied(key: String, open: OpenMemoryFile, sent: String, answer: MobileMemorySaved): OpenMemoryFile = when (answer.status) {
        MobileMemorySaved.SAVED -> {
            ...
            // The machine's answer, absent included: an instruction file cut back under the CLI's 80% has no meter to show.
            open.copy(
                savedText = sent, revision = answer.revision, existed = true, conflict = null, error = null, saved = !kept,
                cap = answer.cap, rulePaths = answer.rulePaths,
            )
        }
    ...
    fun keepMine(open: OpenMemoryFile): OpenMemoryFile {
        ...
        return open.copy(conflict = null, revision = theirs.revision, savedText = theirs.content, cap = theirs.cap, rulePaths = theirs.rulePaths)
    }
    ...
    fun loadTheirs(key: String, open: OpenMemoryFile): OpenMemoryFile {
        ...
        drafts.remove(key)
        return open.copy(
            conflict = null, text = theirs.content, savedText = theirs.content, revision = theirs.revision,
            cap = theirs.cap, rulePaths = theirs.rulePaths,
        )
    }
