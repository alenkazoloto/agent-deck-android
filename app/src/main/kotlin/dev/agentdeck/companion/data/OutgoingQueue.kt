...
data class OutgoingSend(
     ...
     */
    val lane: String get() = key ?: "new:$vendor:$projectPath"
...
data class OutgoingQueue(val items: List<OutgoingSend> = emptyList()) {
    ...
    companion object {
        ...
        /** The user's own Retry: it runs now, whatever the backoff had decided. */
        ...
        fun attempting(item: OutgoingSend): OutgoingSend = item.copy(inFlight = true)
         ...
         * parked only because delivery was uncertain. A refusal is never in this set — the
         * machine answered, and a reconnect does not change its answer.
         */
        fun resumable(queue: OutgoingQueue): OutgoingQueue = OutgoingQueue(
            queue.items.map { if (it.parked && it.uncertain && !it.refused) resumed(it) else it },
        )
