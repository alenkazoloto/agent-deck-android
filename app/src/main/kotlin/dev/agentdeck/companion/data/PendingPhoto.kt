...
object TextFileAttachment {
    ...
    /** The sentence for a Zstandard file this phone will not upload, or null when the machine may be asked. */
    fun zstdRefusal(bytes: ByteArray): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        bytes.size > MobileAttachment.MAX_ZSTD_BYTES ->
            "That file is over ${MobileAttachment.MAX_ZSTD_BYTES / (1024 * 1024)} MB — only smaller Zstandard files can be attached"
        else -> null
    }

    /** The sentence for a 7z archive this phone will not upload, or null when the machine may be asked. */
    fun sevenZipRefusal(bytes: ByteArray): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        bytes.size > MobileAttachment.MAX_7Z_BYTES ->
            "That file is over ${MobileAttachment.MAX_7Z_BYTES / (1024 * 1024)} MB — only smaller 7z archives can be attached"
        else -> null
    }

    /** The sentence for a RAR archive this phone will not upload, or null when the machine may be asked. */
    fun rarRefusal(bytes: ByteArray): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        bytes.size > MobileAttachment.MAX_RAR_BYTES ->
            "That file is over ${MobileAttachment.MAX_RAR_BYTES / (1024 * 1024)} MB — only smaller RAR archives can be attached"
        else -> null
    }

    /** The sentence for a file this phone will not upload, or null when the machine may be asked. */
    fun refusal(bytes: ByteArray, pdfSupported: Boolean = false, zipSupported: Boolean = false, gzipSupported: Boolean = false, tarSupported: Boolean = false, xzSupported: Boolean = false, bzip2Supported: Boolean = false, zstdSupported: Boolean = false, sevenZipSupported: Boolean = false, rarSupported: Boolean = false): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        ...
        !MobileAttachment.looksLikeText(bytes) -> {
            ...
            val kinds = listOfNotNull(
                ...
                ("a Zstandard file" to "Zstandard files").takeIf { zstdSupported },
                ("a 7z archive" to "7z archives").takeIf { sevenZipSupported },
                ("a RAR archive" to "RAR archives").takeIf { rarSupported },
            )
