...
object TextFileAttachment {
    ...
    fun refusal(bytes: ByteArray, pdfSupported: Boolean = false, zipSupported: Boolean = false): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        ...
        !MobileAttachment.looksLikeText(bytes) -> when {
            pdfSupported && zipSupported -> "That file is not UTF-8 text, a PDF or a ZIP archive — only photos, text files, PDFs and ZIP archives can be attached"
            pdfSupported -> "That file is not UTF-8 text or a PDF — only photos, text files and PDFs can be attached"
            zipSupported -> "That file is not UTF-8 text or a ZIP archive — only photos, text files and ZIP archives can be attached"
            else -> "That file is not UTF-8 text — only photos and text files can be attached"
        }
        else -> null
    }
