...
object TextFileAttachment {
    ...
    fun refusal(bytes: ByteArray, pdfSupported: Boolean = false): String? = when {
        bytes.isEmpty() -> "That file is empty or could not be read"
        ...
        !MobileAttachment.looksLikeText(bytes) -> if (pdfSupported) "That file is not UTF-8 text or a PDF — only photos, text files and PDFs can be attached"
