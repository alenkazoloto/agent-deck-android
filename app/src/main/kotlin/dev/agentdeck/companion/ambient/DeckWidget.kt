...
/**
 * The waiting count and the top row, on the home screen.
 *
...
class DeckWidget : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun render(context: Context): RemoteViews {
            ...
            val waiting = snapshot?.badgeCount ?: 0
            val top = snapshot?.rows
                ?.filter { it.attention == com.github.claudeagents.core.SessionAttentionState.WAITING_ON_YOU }
            ...
            views.setTextViewText(
                ...
                when {
                    ...
                    waiting == 1 -> "1 waiting on you"
                    else -> "$waiting waiting on you"
                },
            )
            views.setTextViewText(
                R.id.widget_top,
                top?.title?.ifBlank { "(no title)" }
                    ?: machine?.machineName?.ifBlank { "Agent Deck" }
                    ?: "Tap to pair",
            )
