...
object DeckFixtures {
     ...
     * carries the machine's refusal under the sheet, which stays open.
     */
    fun revertSheet(name: String): dev.agentdeck.companion.data.RevertSheet? {
        if (name != "convo-revert" && name != "convo-revert-failed") return null
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        "convo-revert", "convo-revert-failed", "convo-revert-list" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let { state ->
            state.copy(
                hello = HELLO.copy(
                    capabilities = HELLO.capabilities + MobileProtocol.Capability.REVIEW +
    ...
    fun names(): List<String> = listOf(
        ...
        "convo-changes", "convo-changes-diff", "convo-commit", "convo-commit-failed", "convo-commit-unsupported", "convo-revert", "convo-revert-failed", "convo-revert-list", "convo-notes-files", "convo-notes-diff", "convo-notes-list", "convo-notes-editor", "convo-queued", "convo-composer-attachments",
