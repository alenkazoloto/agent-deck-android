...
object DeckFixtures {
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        // A photo and two text files uploaded, the second with a name too long for its chip, on a machine
        // that advertises `attachment-text` — so the paperclip is the photo-or-file menu.
        "convo-composer-files" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let { state ->
            ...
            state.copy(
                ...
                pendingPhotos = mapOf(
                    key to listOf(
                        dev.agentdeck.companion.data.PendingPhoto("photo-1", "Photo · 1.2 MB"),
                        dev.agentdeck.companion.data.PendingPhoto("file-1", dev.agentdeck.companion.data.TextFileAttachment.label("build-2026-09-24.log", 412 * 1024)),
    ...
    fun names(): List<String> = listOf(
        ...
        "convo-permission", "convo-permission-off", "convo-plan", "convo-plan-off", "convo-retry", "convo-claude", "convo-codex", "convo-truncated", "convo-whole", "convo-idle",
