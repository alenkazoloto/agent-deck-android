...
object DeckFixtures {
    ...
    /** Codex's review sheet: the form (`convo-ai-review`), a run going, and its findings. */
    ...
    fun reviewSheet(name: String): dev.agentdeck.companion.data.ReviewSheet? {
        ...
        fun c(author: String, body: String) = com.github.claudeagents.core.mobile.MobileChangeRequestComment(author, body)
        ...
        val review = com.github.claudeagents.core.mobile.MobileChangeRequestReview(
            ...
            threads = listOf(
                t("t1", "src/main/kotlin/Clock.kt:18 · 2 comments", resolvable = true, comments = listOf(
                    c("ana", "Could this default to the system clock so callers do not have to pass one?"),
                    c("dev", "It does — `Clock.System`. Tests hand in their own."),
                )),
                t("t2", "README.md:4 · resolved · 1 comment", resolved = true, resolvable = true, comments = listOf(c("bo", "Typo: \"injectible\"."))),
                t("t3", "On the request · 1 comment", comments = listOf(c("ana", "Looks good once the tests pass."))),
            ),
        ...
        return when (name) {
            "new-chat-worktrees-review-none" -> dev.agentdeck.companion.data.ReviewSheet(
                ...
                drafts = dev.agentdeck.companion.data.ReviewDrafts(mapOf("t1" to "Thanks — adding a test for the default."), "Approving once CI is green."),
            )
        }
    }
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        "new-chat-worktrees-review-button", "new-chat-worktrees-review-github", "new-chat-worktrees-review-gitlab", "new-chat-worktrees-review-none" -> newChat().let { state ->
            state.copy(
                hello = HELLO.copy(
                    capabilities = HELLO.capabilities + MobileProtocol.Capability.WORKTREE_CREATE + MobileProtocol.Capability.WORKTREE_MANAGE +
                        MobileProtocol.Capability.CHANGE_REQUEST_REVIEW,
                ),
            )
        }
        ...
        "new-chat-worktree", "new-chat-worktree-failed", "new-chat-worktree-unsupported" -> newChat().let { state ->
            state.copy(
                hello = if (name == "new-chat-worktree-unsupported") HELLO
                else HELLO.copy(capabilities = HELLO.capabilities + MobileProtocol.Capability.WORKTREE_CREATE),
        ...
        // Codex's `/review` sheet ([aiReview]) over a Codex conversation.
        "convo-ai-review", "convo-ai-review-running", "convo-ai-review-findings" -> conversation(vendor = AgentVendor.CODEX, running = false).let { state ->
        ...
        "convo-notes-diff", "convo-notes-list", "convo-notes-editor" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let { state ->
            state.copy(
                hello = HELLO.copy(capabilities = HELLO.capabilities + MobileProtocol.Capability.REVIEW + MobileProtocol.Capability.REVIEW_NOTES),
                review = REVIEW,
                changesOpenKey = "convo-1",
    ...
    fun names(): List<String> = listOf(
        ...
        "new-chat-run-options", "new-chat-worktree", "new-chat-worktree-failed", "new-chat-worktree-unsupported", "new-chat-worktrees-button", "new-chat-worktrees", "new-chat-worktrees-remove", "new-chat-worktrees-request", "new-chat-worktrees-link", "new-chat-worktrees-review-button", "new-chat-worktrees-review-github", "new-chat-worktrees-review-gitlab", "new-chat-worktrees-review-none", "new-chat-repository-button", "new-chat-repository-clone", "new-chat-repository-publish", "new-chat-repository-cloned", "convo-composer-pills", "convo-waiting", "convo-questions-multi", "convo-tool-results",
        "convo-changes", "convo-changes-diff", "convo-commit", "convo-commit-failed", "convo-commit-unsupported", "convo-revert", "convo-revert-failed", "convo-revert-list", "convo-revert-request", "convo-revert-after", "convo-notes-files", "convo-notes-diff", "convo-notes-list", "convo-notes-editor", "convo-queued", "convo-composer-attachments",
        ...
        "fleet-fork-picker", "fleet-pins", "convo-rewind-targets", "convo-rewind-scopes", "convo-rewind-code",
        "convo-ai-review", "convo-ai-review-running", "convo-ai-review-findings",
