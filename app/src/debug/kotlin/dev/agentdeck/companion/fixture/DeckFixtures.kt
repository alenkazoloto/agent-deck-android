...
object DeckFixtures {
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        // A photo, two text files (the second with a name too long for its chip) and a PDF uploaded, on a machine
        // that advertises `attachment-text` and `attachment-pdf` — so the paperclip is the photo-or-file menu.
            ...
            state.copy(
                hello = HELLO.copy(
                    capabilities = HELLO.capabilities +
                        ...
                        MobileProtocol.Capability.ATTACHMENT_PDF,
                ),
    ...
    fun names(): List<String> = listOf(
        ...
        "convo-permission", "convo-permission-off", "convo-plan", "convo-plan-off", "convo-goal", "convo-retry", "convo-claude", "convo-codex", "convo-truncated", "convo-whole", "convo-idle",
