...
object DeckFixtures {
    ...
    fun names(): List<String> = listOf(
        ...
        "convo-changes", "convo-changes-diff", "convo-queued", "convo-composer-attachments",
        ...
        "fleet-organized", "fleet-organized-done", "fleet-message-search", "fleet-delete-confirm",
    )
    ...
    private fun organized(scope: FleetScope): DeckState = fleet(backlog = 4, sort = FleetSort.RECENT).let { state ->
        ...
        state.copy(
            snapshot = snapshot.copy(rows = snapshot.rows.map {
                when (it.key) {
                    "done-3" -> it.copy(pinned = true, title = "Release checklist")
                    "recent-2" -> it.copy(done = true)
                    else -> it
                }
            }),
            hello = HELLO.copy(capabilities = HELLO.capabilities + MobileProtocol.Capability.SESSION_ACTIONS + MobileProtocol.Capability.SESSION_EXPORT + MobileProtocol.Capability.SESSION_DELETE),
