...
object DeckFixtures {
     ...
     * with an unsaved edit (`settings-memory-editor`) and the same edit after the machine's file moved on
    ...
    fun memorySheet(name: String): dev.agentdeck.companion.data.MemorySheet? {
        ...
        fun file(writable: Boolean, edited: String, conflict: com.github.claudeagents.core.mobile.MobileMemoryFile? = null) =
            dev.agentdeck.companion.data.OpenMemoryFile(
                id = "Instructions:Project (CLAUDE.md)", label = "Project (CLAUDE.md)", writable = writable, existed = true,
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        "settings-memory-files", "settings-memory-readonly", "settings-memory-editor", "settings-memory-conflict" ->
            settings().copy(hello = HELLO.copy(capabilities = HELLO.capabilities + MobileProtocol.Capability.MEMORY_FILES))
