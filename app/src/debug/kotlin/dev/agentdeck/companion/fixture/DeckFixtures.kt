...
object DeckFixtures {
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        // A run with work left in the background (P-background-tasks): the desk's `/tasks` rows, read-only.
        "convo-background-tasks" -> conversation(vendor = AgentVendor.CLAUDE, running = true).let {
            it.copy(
                transcript = it.transcript!!.copy(
                    backgroundTasks = listOf(
                        MobileBackgroundTask("Agent", "Review the payment retry changes"),
                        MobileBackgroundTask("Shell command", "npm run test:integration"),
                        MobileBackgroundTask("Monitor", null),
                    ),
                ),
            )
        }
    ...
    fun names(): List<String> = listOf(
        ...
        "settings-usage", "settings-keep-awake", "settings-keep-awake-off", "settings-keep-awake-review", "settings-new-chat-mode", "usage", "usage-unmeasured", "usage-limit", "usage-spend-defaults", "usage-spend-defaults-sheet", "usage-export", "usage-set-active", "scheduled-after-reset", "convo-limit",
