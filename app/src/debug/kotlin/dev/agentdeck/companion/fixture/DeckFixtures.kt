...
import dev.agentdeck.companion.DeckViewModel
...
object DeckFixtures {
    ...
    fun byName(name: String): DeckState? = when (name) {
        ...
        // A row's Duplicate: the create dialog open on the nightly sweep's prompt, project, agent, account and
        // run choices. `scheduled` is the control — its Duplicate shows on this one row only, since the paused
        // chat-bound row and the running one are not copied.
        "scheduled-duplicate" -> scheduled().copy(
            hello = HELLO_RUN_OPTIONS,
            scheduleDuplicate = MobileScheduleEditDetail(
                "s1", "Run the nightly regression sweep", "/Users/dev/Plugin", null, NOW + DAY, DAY, null, "opus",
                "CLAUDE", "default", null, "Europe/Amsterdam", editable = true, canRepeat = true,
                effort = "medium", runOptionsEditable = true,
            ),
            drafts = mapOf(DeckViewModel.scheduleDuplicateDraftKey("s1") to "Run the nightly regression sweep"),
        )
        "settings" -> settings()
        ...
        "settings-pin-new-chats" -> settings().copy(keepAwake = true, pinNewChats = true)
        "settings-branch-mentions" -> settings().copy(keepAwake = true, branchMentions = false)
        "settings-skill-reminder" -> settings().copy(keepAwake = true, skillReminder = true)
        "settings-skills-in-slash" -> settings().copy(keepAwake = true, skillsInSlash = true)
        // Same gate: `settings` is the negative control. `convo-plain-text` is a finished chat with the switch
        // off; `convo-claude`, the same answer with the machine's default, is its formatted control.
        "settings-chat-markup" -> settings().copy(keepAwake = true, chatMarkup = false)
        "convo-plain-text" -> conversation(vendor = AgentVendor.CLAUDE, running = false, turns = 2).copy(chatMarkup = false)
        // Same gate: `settings` (no `jumpOnSend`) is the negative control.
        "settings-jump-on-send" -> settings().copy(keepAwake = true, jumpOnSend = true)
        // Same gate: `settings` (no `quickReplies`) is the negative control. The chips show only over an empty
        // message box, so both chats have no draft: `convo-no-quick-replies` has the switch off,
        // `convo-quick-replies` is its chipped control.
        "settings-quick-replies" -> settings().copy(keepAwake = true, quickReplies = false)
        "convo-no-quick-replies" -> conversation(vendor = AgentVendor.CLAUDE, running = false).copy(drafts = emptyMap(), quickReplies = false)
        "convo-quick-replies" -> conversation(vendor = AgentVendor.CLAUDE, running = false).copy(drafts = emptyMap(), quickReplies = true)
        // Same gate: `settings` (no `contextWarning`) is the negative control. The banner rides the page's
        // percentage and the switch: `convo-context-warning` is 84% full with it on, `convo-context-warning-off` the
        // same page with it off, and `convo-context-quiet` a 64% page with it on — under the 80% line.
        "settings-context-warning" -> settings().copy(keepAwake = true, contextWarning = true)
        "convo-context-warning" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let {
            it.copy(drafts = emptyMap(), contextWarning = true, transcript = it.transcript!!.copy(contextPct = 84))
        }
        "convo-context-warning-off" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let {
            it.copy(drafts = emptyMap(), contextWarning = false, transcript = it.transcript!!.copy(contextPct = 84))
        }
        "convo-context-quiet" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let {
            it.copy(drafts = emptyMap(), contextWarning = true, transcript = it.transcript!!.copy(contextPct = 64))
        }
        // Same gate: `settings` (no `cacheClock`) is the negative control. The words ride the page the machine
        // made, so `convo-cache-clock` carries them and `convo-idle` (switch off, nothing sent) is its control.
        "settings-cache-clock" -> settings().copy(keepAwake = true, cacheClock = true)
        "convo-cache-clock" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let {
            it.copy(transcript = it.transcript!!.copy(cacheClock = "cache 12m"))
        }
        "convo-cache-cold" -> conversation(vendor = AgentVendor.CLAUDE, running = false).let {
            it.copy(transcript = it.transcript!!.copy(cacheClock = "cache cold"))
        }
        // Same gate: `settings` (no `pinnedTodos`) is the negative control. `convo-tasks` is the same open list
        // with the switch unread (nothing pinned); `convo-tasks-pinned` has it on, `convo-tasks-pinned-off` off.
        "settings-pinned-todos" -> settings().copy(keepAwake = true, pinnedTodos = true)
        "convo-tasks-pinned" -> conversation(vendor = AgentVendor.CLAUDE, todos = TODOS).copy(pinnedTodos = true)
        "convo-tasks-pinned-off" -> conversation(vendor = AgentVendor.CLAUDE, todos = TODOS).copy(pinnedTodos = false)
        // The skills sheet opens on `skillsList`; it offers "Available plugins" only where the machine advertises
        // `plugin-install`, so `settings-skills` (the same list without it) is the negative control.
        "settings-skills", "settings-skills-install" -> settings().copy(
            hello = HELLO.copy(
                capabilities = HELLO.capabilities + MobileProtocol.Capability.SKILLS_LIST + MobileProtocol.Capability.PLUGIN_TOGGLE +
                    MobileProtocol.Capability.PLUGIN_UPDATE + listOfNotNull(MobileProtocol.Capability.PLUGIN_INSTALL.takeIf { name == "settings-skills-install" }),
            ),
        )
        ...
        // Drawn once a phone holding the grant has read the desk's values, so `settings` is its negative control.
        "settings-auto-handoff" -> settings().copy(keepAwake = true, autoHandoff = com.github.claudeagents.core.mobile.MobileAutoHandoff(enabled = true, sessionPercent = 90, weeklyPercent = 95, multiAccount = true))
        ...
        // Both pickers are drawn once the machine answered with the desk's pair and its lists (empty with no
        // Codex), so `settings` is their negative control; this frame is a long, detailed pair.
        "settings-codex-output" -> settings().copy(
            keepAwake = true,
            codexOutput = com.github.claudeagents.core.mobile.MobileCodexOutput(
                "high", "detailed",
                listOf("default" to "Codex default", "low" to "Low", "medium" to "Medium", "high" to "High")
                    .map { (slug, label) -> com.github.claudeagents.core.mobile.MobileModelOption(slug, label) },
                listOf("default" to "Codex default", "auto" to "Auto", "concise" to "Concise", "detailed" to "Detailed", "none" to "None")
                    .map { (slug, label) -> com.github.claudeagents.core.mobile.MobileModelOption(slug, label) },
            ),
        )
        "usage" -> usage()
    ...
    /** The list the skills sheet is opened on by `settings-skills` and `settings-skills-install`: one installed plugin, two on offer. */
    fun skillsList(name: String): com.github.claudeagents.core.mobile.MobileSkillsList? {
        if (name != "settings-skills" && name != "settings-skills-install") return null
        val alpha = com.github.claudeagents.core.mobile.MobileSkillsProject("/work/alpha", "alpha")
        val review = com.github.claudeagents.core.mobile.MobileSkillRow(
            "plugin", "review", "Review pull requests.", "acme · user", "Enabled", "v1.2.0 · 3 skills · 1 hook", "review@acme", true, true,
        )
        return com.github.claudeagents.core.mobile.MobileSkillsList(
            "/work/alpha", listOf(alpha), listOf(review, com.github.claudeagents.core.mobile.MobileSkillRow("marketplace", "acme", "", "GitHub", "", "Updates itself")),
            available = listOf(
                com.github.claudeagents.core.mobile.MobileAvailablePlugin("lint@acme", "lint", "acme", "Lint every changed file before a commit and say what it fixed.", 1234),
                com.github.claudeagents.core.mobile.MobileAvailablePlugin("deployer@acme", "deployer", "acme", "Ship a build to staging from the chat."),
            ),
        )
    }
    ...
    fun names(): List<String> = listOf(
        ...
        "scheduled", "scheduled-background-runs", "scheduled-no-create", "scheduled-reset", "scheduled-edit-toggles", "scheduled-edit-plain", "scheduled-duplicate", "settings", "settings-two-machines",
        ...
        "settings-usage", "settings-keep-awake", "settings-keep-awake-off", "settings-keep-awake-review", "settings-prompt-suggestions", "settings-restricted-mode", "settings-questions-keep-working", "settings-continue-interrupted", "settings-pin-new-chats", "settings-branch-mentions", "settings-skill-reminder", "settings-skills-in-slash", "settings-chat-markup", "convo-plain-text", "settings-jump-on-send", "settings-quick-replies", "convo-no-quick-replies", "convo-quick-replies", "settings-context-warning", "convo-context-warning", "convo-context-warning-off", "convo-context-quiet", "settings-cache-clock", "convo-cache-clock", "convo-cache-cold", "settings-pinned-todos", "convo-tasks-pinned", "convo-tasks-pinned-off", "settings-skills", "settings-skills-install", "settings-chat-auto-hide", "settings-chat-auto-compact", "settings-auto-handoff", "settings-auto-review", "settings-new-chat-mode", "settings-codex-output", "usage", "usage-unmeasured", "usage-limit", "usage-spend-defaults", "usage-spend-defaults-sheet", "usage-export", "usage-set-active", "usage-account-edit", "scheduled-after-reset", "convo-limit",
