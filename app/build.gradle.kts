...
val sharedProtocolFiles = listOf(
    ...
    // The bridge's own refusal sentences, split out of MobileProtocol.kt to keep it under the 1,500-line cap.
    "com/github/claudeagents/core/mobile/MobileRefusal.kt",
    // ACP conversations in Chats (M5 / P07): the fleet array the phone folds into its rows.
    "com/github/claudeagents/core/mobile/MobileAcpSession.kt",
    // Settings › Orchestration (P35), read-only: the team boards and workflow runs the phone lists.
    "com/github/claudeagents/core/mobile/MobileOrchestration.kt",
    // Resources › Badges (P39): the gallery the phone lists and the share line it hands to the share sheet.
    "com/github/claudeagents/core/mobile/MobileBadges.kt",
    // Resources › Extensions (P38), read-only: the installed extensions the phone lists.
    "com/github/claudeagents/core/mobile/MobileExtensions.kt",
    // Resources › Memory and instructions (P29): the desk's Memory tab files, read and saved as text.
    "com/github/claudeagents/core/mobile/MobileMemory.kt",
    // Resources › MCP servers (P31): the redacted, read-only server list.
    "com/github/claudeagents/core/mobile/MobileMcp.kt",
    // Settings › Machine › Agents' ACP rows: connection state, Connect and Sign in.
    "com/github/claudeagents/core/mobile/MobileAcpAgents.kt",
    // `/prompts` and `/saveprompt`: the desk's saved prompt library, listed and added to from the composer.
    "com/github/claudeagents/core/mobile/MobilePromptLibrary.kt",
    // Resources › Skills and agents (P30): the read-only skill, command and subagent rows.
    "com/github/claudeagents/core/mobile/MobileSkills.kt",
    // A Task/Agent call's own conversation, read-only, opened from its tool row.
    "com/github/claudeagents/core/mobile/MobileSubagentThread.kt",
    // Settings › Machine's "Keep awake while an agent works", one value with the desk's.
    "com/github/claudeagents/core/mobile/MobileMachineBehaviour.kt",
    // Settings › Machine's "Start new chats in", the desk's pin as one value.
    "com/github/claudeagents/core/mobile/MobileActiveAccount.kt",
    // Usage › Plans' account "Rename" and "Remove", under the per-phone grant.
    "com/github/claudeagents/core/mobile/MobileAccountEdit.kt",
    "com/github/claudeagents/core/mobile/MobileNewChatDefaults.kt",
    // Settings › Machine's "Suggest the next prompt", the desk's switch under the per-phone grant.
    "com/github/claudeagents/core/mobile/MobilePromptSuggestions.kt",
    // Settings › Machine's "Restricted mode for Claude chats", the desk's switch under the per-phone grant.
    "com/github/claudeagents/core/mobile/MobileRestrictedMode.kt",
    // Settings › Machine's "Let agents keep working while a question waits", the same switch under the same grant.
    "com/github/claudeagents/core/mobile/MobileQuestionsKeepWorking.kt",
    // Settings › Machine's "Continue an interrupted step automatically", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileContinueInterrupted.kt",
    // Settings › Machine's "Pin new chats", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobilePinNewChats.kt",
    // Settings › Machine's "Skills in the / menu", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileSkillsInSlash.kt",
    // Settings › Machine's "Formatting in messages", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileChatMarkup.kt",
    // Settings › Machine's "Scroll to the newest message when I send", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileJumpOnSend.kt",
    // Settings › Machine's "Branches after @", one value with the desk's "Complete git branches after @", no grant.
    "com/github/claudeagents/core/mobile/MobileBranchMentions.kt",
    // Settings › Machine's "Context warning", one value with the desk's "Warn when a chat context is nearly full", no grant.
    "com/github/claudeagents/core/mobile/MobileContextWarning.kt",
    // Settings › Machine's "Quick replies", one value with the desk's "Show quick replies", no grant.
    "com/github/claudeagents/core/mobile/MobileQuickReplies.kt",
    // Settings › Machine's "Prompt cache clock", one value with the desk's "Show prompt cache clock", no grant.
    "com/github/claudeagents/core/mobile/MobileCacheClock.kt",
    // Settings › Machine's "Pinned todo list", one value with the desk's "Pin the agent's todo list", no grant.
    "com/github/claudeagents/core/mobile/MobilePinnedTodos.kt",
    // Settings › Machine's "Mention matching skills to Claude", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileSkillReminder.kt",
    // Settings › Machine's Codex answer length and reasoning summary, one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileCodexOutput.kt",
    // Settings › Machine's "Hide chats I haven't opened in N days", one value with the desk's, no grant.
    "com/github/claudeagents/core/mobile/MobileChatAutoHide.kt",
    // Settings › Machine's "Compact chats idle for N days", one value with the desk's, under the per-phone grant.
    "com/github/claudeagents/core/mobile/MobileChatAutoCompact.kt",
    // Settings › Machine's "Continue on another account near the usage limit", the desk's own values, under the per-phone grant.
    "com/github/claudeagents/core/mobile/MobileAutoHandoff.kt",
    // Settings › Machine's two "Review changes automatically" switches, the desk's own under the same grant.
    "com/github/claudeagents/core/mobile/MobileAutoReview.kt",
    // Resources › Preview (P34): the desk's Preview tab as a host-rendered picture with typed intents back.
    "com/github/claudeagents/core/mobile/MobilePreview.kt",
    "com/github/claudeagents/core/mobile/MobileTaskStop.kt",
    "com/github/claudeagents/core/mobile/MobileScheduleEdit.kt",
    // Pin, Done, reopen and rename (M3): the request the phone sends and the state it reads back.
    "com/github/claudeagents/core/mobile/MobileSessionAction.kt",
    // Message search (M3): the page the phone asks for and the hits and coverage it reads back.
    "com/github/claudeagents/core/mobile/MobileSessionSearch.kt",
    // Share (M3): the desk's export text and file name, and the bound the phone reads it under.
    "com/github/claudeagents/core/mobile/MobileSessionExport.kt",
    // Delete (M3): the preview the phone confirms, its fingerprint, and the machine's verdict.
    "com/github/claudeagents/core/mobile/MobileSessionDelete.kt",
    // Fork (M3): the messages a chat forks from, the fork request, and the new chat's key.
    "com/github/claudeagents/core/mobile/MobileSessionFork.kt",
    // Handoff: "Continue on <account>" — the request, the copy's answer and the desk's continuation prompt.
    "com/github/claudeagents/core/mobile/MobileSessionHandoff.kt",
    // Rewind (M4): the messages a chat goes back to before, each one's scopes, and the conversation half's outcome.
    "com/github/claudeagents/core/mobile/MobileSessionRewind.kt",
    // AI review (M4): Codex's `/review` — its target, the running state and the findings.
    "com/github/claudeagents/core/mobile/MobileAiReview.kt",
    // Working diff (M4): a Codex chat's `/diff` — every uncommitted file and its patch.
    "com/github/claudeagents/core/mobile/MobileWorkingDiff.kt",
    "com/github/claudeagents/core/mobile/MobileTranscript.kt",
    // `MobileToolCall.result` is typed with it, so rule 16 fails the moment it is not here.
    "com/github/claudeagents/core/mobile/MobileToolResult.kt",
    // The Changes tab's whole wire: the checklist, one file's hunks, and the tick. Listed for
    // the reason above — the phone decodes all three through the plugin's own encoder.
    "com/github/claudeagents/core/mobile/MobileReview.kt",
    // Commit (M4): the files a commit would record, the confirmed request and git's verdict.
    "com/github/claudeagents/core/mobile/MobileReviewCommit.kt",
    // Revert (M4): what the desk's session revert would restore/delete/skip, the request, the outcome.
    "com/github/claudeagents/core/mobile/MobileReviewRevert.kt",
    // Review notes (M4): the diff's saved notes, the write and the `@review-notes:` token it attaches.
    "com/github/claudeagents/core/mobile/MobileReviewNotes.kt",
    // New chat in a worktree (M4): the desk dialog's suggestion and bases, the create and its state.
    "com/github/claudeagents/core/mobile/MobileWorktree.kt",
    // Manage worktrees (M4): the desk menu's rows and refusals, and the merge/remove/prune write.
    "com/github/claudeagents/core/mobile/MobileWorktreeFleet.kt",
    // Commit staged (M4): the index a commit would record, the commit or message-write, and its state.
    "com/github/claudeagents/core/mobile/MobileCommitStaged.kt",
    // Change-request review (M4): the worktree branch's open request and threads, and the reply/resolve/verdict write.
    "com/github/claudeagents/core/mobile/MobileChangeRequestReview.kt",
    // Clone and Publish repository (M4): the desk dialogs' hosts and directories, the write and its state.
    "com/github/claudeagents/core/mobile/MobileRepositorySetup.kt",
    // The `@` popup's answer and the one route that carries a file. Listed for the reason above:
    // the phone decodes both through the plugin's own encoder, and the attachment file's bounds
    // — the cap, the expiry, the per-send count — are read by the client that must not exceed
    // them and by the machine that refuses when it does.
    "com/github/claudeagents/core/mobile/MobileFiles.kt",
    // The `/` and `$` popup's catalogue and the caret rule that opens it: the phone decodes the
    // plugin's own rows and filters them with the `matching` that `MobileCommandsTest` pins.
    "com/github/claudeagents/core/mobile/MobileCommands.kt",
    // The context grid the chat's gauge opens: the rows the machine words, and the route and notice both ends use.
    "com/github/claudeagents/core/mobile/MobileContextBreakdown.kt",
    // The desk's `/btw`: a question about a chat answered on a copy that is saved nowhere, its asks and the route both ends use.
    "com/github/claudeagents/core/mobile/MobileSideQuestion.kt",
    // A chat's additional working directories (`--add-dir`): the list, the typed change and the desk's
    // refusal sentence both ends read, under the same per-phone grant as tool-permission decisions.
    "com/github/claudeagents/core/mobile/MobileSessionDirs.kt",
    // Which MCP servers a Claude chat may use: the list, the whole-selection save and the desk's refusal both ends read.
    "com/github/claudeagents/core/mobile/MobileSessionMcp.kt",
    // A Codex chat's communication style, web-search mode and config profile: the three controls, the one-value
    // change, the per-control note on what the phone's own replies use and the desk's refusal both ends read.
    "com/github/claudeagents/core/mobile/MobileSessionCodex.kt",
    // A chat's own spend limits: the form as text, the typed save and the desk's refusal sentence both ends read,
    // under the same per-phone grant.
    "com/github/claudeagents/core/mobile/MobileSessionSpend.kt",
    // The desk-only slash names the phone answers with its own controls, and the bare-line rule
    // that decides when a sent line is one of them (`PhoneDeskCommandParityTest`).
    "com/github/claudeagents/core/mobile/MobileDeskCommands.kt",
    // The `#` popup's rows and the caret rule that opens it, so both ends agree on what `#123` is.
    "com/github/claudeagents/core/mobile/MobileIssues.kt",
    "com/github/claudeagents/core/mobile/MobileAttachment.kt",
    // The `:` emoji completion: the desk's own shortcode table, ranking and `:name:` swap, so a
    // phone prompt reads what the same keystrokes write on the desk. Its table is in
    // `sharedResourceFiles`.
    "com/github/claudeagents/core/composer/EmojiCompletion.kt",
    "com/github/claudeagents/core/composer/WordStart.kt",
    // The composer stash: the desk's queue, cap, ids, snippet and persisted form, so a prompt
    // parked on the phone behaves as one parked at the desk (newest first, 20 deep, legacy-safe).
    "com/github/claudeagents/core/composer/ComposerStashQueue.kt",
    // The goal strip's condition text and the `/goal clear` its Clear puts in the message box.
    "com/github/claudeagents/core/sessions/ChatGoal.kt",
    // The suggested-reply strip's rule for when a prediction is worth a row (`shouldShow`) and
    // how it is cleaned and elided, one function for the desk's row and the phone's.
    "com/github/claudeagents/core/composer/PromptSuggestions.kt",
    // The phone's own log, sent to the machine on request: the route, the cap the phone trims to
    // and the one the machine refuses above are one number read by both.
    "com/github/claudeagents/core/mobile/MobilePhoneLog.kt",
    // The image route's name and bounds, and the one rule both ends need about which markdown
    // destinations are worth asking the machine for — see `MobileImage.isLocalSource`.
    "com/github/claudeagents/core/mobile/MobileImage.kt",
    // The addresses a paired phone re-reads after the machine's own list changed.
    "com/github/claudeagents/core/mobile/MobileHosts.kt",
    // The Usage screen's whole wire. Listed for the reason above, and for one more: the cost
    // figures carry two honesty flags (`costKnown`, `costEstimated`) that decide whether the
    // phone paints `$`, `~$` or `≥$` — a second decoder would be a second opinion about
    // whether a number is a total or a floor.
    "com/github/claudeagents/core/mobile/MobileUsage.kt",
    "com/github/claudeagents/core/mobile/MobileUsageExport.kt",
    "com/github/claudeagents/core/mobile/MobileInsights.kt",
    // Not a wire format — a *rendering* one. `$12.34` / `~$12.34` / `≥$12.34` / `<$0.01`
    // is the plugin's statement about how much it knows, and a phone with its own
    // rounding would print `$0.00` over a day that had spend. Pure Kotlin; nothing to strip.
    "com/github/claudeagents/core/spend/Money.kt",
    ...
    // `SessionSummary.promptCache` is typed `PromptCache.Stats`, so the phone has to name it.
    // Listed rather than moved out of `Model.kt`, for the reason `HealthStat` was moved *in*:
    // this file is already a leaf — pure Kotlin plus gson, which the shared set already needs
    // for `JsonExt.kt` — so sharing it costs the Android build nothing it was not paying.
    "com/github/claudeagents/core/spend/PromptCache.kt",
)
...
/** Plugin resources a shared source reads with `getResourceAsStream`, copied the same way. */
val sharedResourceFiles = listOf("emoji-shortcodes.txt")

abstract class ExtractSharedResources : DefaultTask() {
    @get:InputFiles
    @get:PathSensitive(PathSensitivity.RELATIVE)
    abstract val sourceFiles: ConfigurableFileCollection
    ...
    fun extract() {
        ...
        val missing = relativePaths.get().filterNot { File(out, it).isFile }
        require(missing.isEmpty()) { "Shared resources missing from the plugin tree: $missing" }
    }
}

val syncSharedResources = tasks.register<ExtractSharedResources>("syncSharedResources") {
    description = "Copies the plugin resources the shared core sources read."
    val root = rootProject.layout.projectDirectory.dir("../src/main/resources")
    relativePaths.set(sharedResourceFiles)
    sourceFiles.from(root.asFileTree.matching { include(sharedResourceFiles) })
    outputDir.set(layout.buildDirectory.dir("generated/shared-protocol/resources"))
}
...
androidComponents {
    onVariants { variant ->
        ...
        variant.sources.resources?.addGeneratedSourceDirectory(
            syncSharedResources,
            ExtractSharedResources::outputDir,
        )
    }
}
...
dependencies {
    ...
    // 47,063 GitHub stars, pushed 2026-09-08 — checked 2026-09-08. Test-only HTTPS bridge.
    androidTestImplementation("com.squareup.okhttp3:mockwebserver:4.12.0")
    // 47,063 GitHub stars, pushed 2026-09-08 — checked 2026-09-08. Test-only certificate minting.
