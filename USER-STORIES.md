...
| M33 | Run my own commands and reuse an earlier prompt from the phone. | Typing `/` at the start of a Claude prompt lists that chat account's own commands, skills and `/compact`, and `$` in a Codex prompt lists its skills; picking one writes it with a space; desk-only commands such as `/model` are not offered, because a phone send reaches the CLI as typed. An empty composer offers "Earlier prompts": this project's recent prompts from the desk and the phone, newest first; tapping one puts it back into the composer. | `ComposerCommandsInteractionTest`, `MobileCommandsWiringTest`, `MobileCommandsTest` |
