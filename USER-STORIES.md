...
| M17 | Keep working with cached conversations when the machine is unreachable. | Cached text and draft remain readable; Back works; when the bridge returns the saved draft can be sent and consumed. | `unreachableMachineKeepsCachedChatAndDraftNavigable` |
