...
| M47 | Try another direction without losing this chat. | A finished Claude row's sheet offers New chat from here…: I pick one of my messages, and a new chat opens holding everything before it, with that message in its composer. A Codex row offers Branch chat, which copies the whole thread. The original chat is unchanged, and the desk lists the copy as “… (fork)”. A running chat and an older machine offer neither. | `SessionForkInteractionTest`, `SessionForkFlowTest`, `MobileSessionForkWiringTest`. |
